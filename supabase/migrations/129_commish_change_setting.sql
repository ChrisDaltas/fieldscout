...
-- | team_count                 | column  | REFUSED | seats and a schedule exist for N (F28's floor lives in 118, pre-draft) |
...
-- file is retired.** 125-128 are merged and NOT yet pushed (hosted tops out at
...
AS $$
  SELECT CASE
    ...
    WHEN p_key IN ('team_count')           THEN jsonb_build_object('storage', 'column', 'class', 'refused', 'refused_why',
                                  'seats (teams / league_members) and a schedule already exist for the stored count; pre-draft the route is update_league_settings (F28''s seat floor lives there, 118:2499-2510); post-draft a franchise is added with add_placeholder_seat or removed with remove_manager, never by moving this number')
