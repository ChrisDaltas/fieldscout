...
select has_function('public', 'draft_nomination_order_internal',
  array['uuid', 'integer', 'text', 'jsonb', 'jsonb', 'uuid', 'text'],
  'draft_nomination_order_internal(...) exists — §8.3/§7.3.8 nomination-order resolution');
select ok(
...
select ok(
  ...
  and not has_function_privilege('anon',
        'public.draft_nomination_order_internal(uuid,integer,text,jsonb,jsonb,uuid,text)', 'EXECUTE')
