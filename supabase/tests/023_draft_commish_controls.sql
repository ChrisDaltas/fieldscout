...
--    Worlds:
--      LM b4…a1  main controls (REAL draft_start; u01 commish
--                @cc_user_01, u02 co-commish @cc_user_02, u03/u04 managers,
...
-- Actor names in the message pins below are the users' HANDLES, minted by
-- handle_new_user from the signup metadata above (u01 -> cc_user_01, etc.).
-- 077 dropped profiles.display_name, so draft_actor_name() resolves to the
-- username — there is no name to plant, and the fixture UPDATEs that used to
-- sit here would not parse.
