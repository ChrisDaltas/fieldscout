...
--      checked against ALL SIX shipped templates as a set equality — each maps
...
-- ===========================================================================
select is(
  (select count(*)::int from t_ids), 6,
  'B1 (premise, F94): six shipped templates — the population §B2/§B3 quantify over. A set that had silently shrunk would make the next two cells vacuously true');
...
select is(
  ...
  'ESPN Full PPR=[0, 1, 7, 14, 18, 28, 35, 46] | ESPN Standard=[0, 1, 7, 14, 18, 28, 35, 46] | Sleeper Full PPR=[0, 1, 7, 14, 21, 28, 35] | Sleeper Standard=[0, 1, 7, 14, 21, 28, 35] | Yahoo Half PPR=[0, 1, 7, 14, 21, 28, 35] | Yahoo Standard=[0, 1, 7, 14, 21, 28, 35]',
  'B2 (§7.3.3.1(b), D44): every shipped template resolves to exactly one points-allowed family, and to the RIGHT one — the two ESPN rows to the 8-cut ESPN list, the four single-model rows to the 7-cut shared list. Detected from the KEY SET; matching on the word "ESPN" in the name would make the fork depend on a display string a commissioner can rename');
...
select is(
  ...
  0, 'B3: …and the family it resolves to REGENERATES that template''s own def_pa_* keys — every one of them, for all six. **This is the whole backward-compatibility argument** (§7.3.3.1(a): "a doc carrying tier_cuts scores the same whether the engine derives from cuts or from derive-stats.ts''s literals"), measured against the shipped rows rather than against a transcription of them');
...
select is(
  ...
  6, 'E7 (058, untouched): templates stay world-readable to this same non-owner member. The additive policy adds a population; it does not narrow one');
