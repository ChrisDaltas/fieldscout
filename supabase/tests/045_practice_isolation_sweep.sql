...
-- 64 `public` tables comes back byte-identical, measured as a whole-row
...
-- on):
--   §A THE INSTRUMENT AND ITS PRECONDITIONS. The 64-table census as a stored
...
--      practice; every one of the 64 tables' count AND whole-row digest
...
select is(
  ...
  'THE CENSUS, as a stored literal: the 64 public tables BY NAME, sorted. A migration that adds, drops or renames one moves this list — re-derive §C''s and §E''s mid-state allowlists in the same PR, deliberately (the F84 enumerate-don''t-glob discipline applied to a schema)');
...
select lives_ok($$ select pg_temp.mp11_take('before') $$,
  'BASELINE: all 64 tables snapshotted (count + whole-row digest each)');
select is((select count(*) from mp11_snap where phase = 'before'), 64::bigint,
  '…one row per table');
...
select lives_ok($$ select pg_temp.mp11_take('after') $$,
  'FINAL: all 64 tables snapshotted again');
...
select lives_ok($$ select pg_temp.mp11_take('a_final') $$,
  '§E FINAL: all 64 tables snapshotted a fourth time');
select is(
