...
select ok(
  ...
  'F112 CLOSED (095/MP.3): draft_mock_cpu_need is REVOKEd from anon AND authenticated. R497 recorded the OPEN grant here on purpose — "so a future REVOKE reddens here and gets read" — and this is that flip, read and taken rather than discovered by an outage. No client path called it (measured: no `.rpc(''draft_mock_cpu_need''` in src/), so D18→D23 applies with no carve-out and it joins draft_autopick_resolve''s posture');
