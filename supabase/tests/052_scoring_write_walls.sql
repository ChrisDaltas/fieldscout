...
select is(
  ...
  'leagues.trg_leagues_scoring_reference_guard=a1c16d02ba52388b91a839c16b82a6fe | leagues.trg_leagues_scoring_rules_valid=8eefa42191daba5ae5d9db5e5ef0c67d | leagues.trg_leagues_snapshot_guard=769a68bcfb6bb599732f68c3c507c94d | scoring_systems.trg_scoring_systems_rules_guard=009f9c68b41f1ff6fb58dfcc3d7725b1',
  'A22 (R635): the BEFORE ROW trigger set on `leagues` and `scoring_systems`, in FIRE ORDER, each with the md5 of its function body. Triggers fire in NAME order, so a trigger that sorts after a wall and assigns the column the wall guards silently un-does it — with an identical catalog. Both halves of that are pinned here. **HAND-CLEARED ONCE, 2026-08-26 by SE.5 (D273/F147):** wall 3''s literal moved 7a5a0273… → a1c16d02… because migration 105 §5 replaced its lock with `FOR SHARE`. The replacement was re-derived by applying **105''s FILE body** inside a rolled-back transaction and hashing the result — never read off the running stack — and the deployed body was first proved to carry a marker only 105''s version has (`D273` present, the `FOR NO KEY UPDATE` statement absent), because a value read out of a database is a claim about the database and not about the repo. The other three literals were re-measured in the same breath and are byte-unchanged, which is what makes "the diff is wall 3''s prosrc alone" a measurement rather than an intention');
