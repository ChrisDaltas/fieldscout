...
select is(
  ...
  'leagues.trg_leagues_scoring_reference_guard=fe39286980888242767eb59059b4e8d1 | leagues.trg_leagues_scoring_rules_valid=8eefa42191daba5ae5d9db5e5ef0c67d | leagues.trg_leagues_snapshot_guard=fcfff41c6be470736401550cedb1fb7b | scoring_systems.trg_scoring_systems_rules_guard=009f9c68b41f1ff6fb58dfcc3d7725b1',
  'A22 (R635) [**HAND-CLEARED AGAIN, 2026-09-02 by L.D1.2 (migration 110 §8 / F143):** trg_leagues_snapshot_guard''s literal moved 769a68bc… → **fcfff41c…** — the F143 detach arm (NEW.scoring_system_id IS NULL in drafting+) joined the guard''s body. Re-derived the same way: 110 §8''s FILE body applied inside a rolled-back transaction and hashed (fcfff41c6be470736401550cedb1fb7b), never read off the running stack; the other three literals re-measured and byte-unchanged; fire order unchanged (the trigger name did not move).] the BEFORE ROW trigger set on `leagues` and `scoring_systems`, in FIRE ORDER, each with the md5 of its function body. Triggers fire in NAME order, so a trigger that sorts after a wall and assigns the column the wall guards silently un-does it — with an identical catalog. Both halves of that are pinned here. **HAND-CLEARED ONCE, 2026-08-26 by SE.5 (D273/F147):** wall 3''s literal moved 7a5a0273… → a1c16d02… → **fe392869…** — once because migration 105 §5 replaced its lock with `FOR SHARE`, and once more in the fix round when R644/R647 corrected two claims INSIDE that body (the deadlock absolute and the mislabelled benchmark). The second move is a comment-only edit and it still reds this cell, which is the pin working: a golden on `prosrc` cannot distinguish a comment from code, and it should not try. The replacement was re-derived by applying **105''s FILE body** inside a rolled-back transaction and hashing the result — never read off the running stack — and the deployed body was first proved to carry a marker only 105''s version has (`D273` present, the `FOR NO KEY UPDATE` statement absent), because a value read out of a database is a claim about the database and not about the repo. The other three literals were re-measured in the same breath and are byte-unchanged, which is what makes "the diff is wall 3''s prosrc alone" a measurement rather than an intention');
  ...
  (select count(*)::int from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'leagues_scoring_reference_guard'
      and regexp_replace(p.prosrc, '--[^\n]*', '', 'g')
          ~ 'WHERE\s+s\.id\s*=\s*NEW\.scoring_system_id\s+FOR SHARE'),
  1, 'K1 (R626, retargeted 2026-08-26 by SE.5 — D273/F147 chose `FOR SHARE`, see 105 §5): wall 3 resolves the reference under a row lock — the lock is on the SELECT that reads the other table''s premise, not merely somewhere in the body. Two concurrent transactions committed a live league in front of the F21 document without it (reproduced by four reviewers in both orderings); with it, both orderings serialise. **The regex is ANCHORED TO THE STATEMENT (F149), and that is the whole cell:** its first form matched the phrase anywhere after the SELECT, so deleting the lock while leaving the words in a comment BELOW it left this cell GREEN — the non-discriminating-probe trap that bit this lane once already, in the mirror direction. It matters because §A22 is a golden pin that every legitimate body edit reds and a human hand-clears, and that hand-clearing window is exactly what K1 is here to cover. **AND THE ANCHOR WAS NOT ENOUGH ON ITS OWN — THE MATCH NOW RUNS OVER `prosrc` WITH LINE COMMENTS STRIPPED (2026-08-26, F159), because this is the THIRD occurrence of one class in this lane.** Measured against the shipped file before the strip: delete the lock and this cell reds (93 total, **2 not-ok** — A22 + K1), which is correct; delete the lock AND add ONE prose line reading `-- The reference is resolved WHERE s.id = NEW.scoring_system_id FOR SHARE (105 §5 / F147).` and it goes GREEN again (93 total, **1 not-ok — A22 alone**). That is a false negative on the cell whose entire job is to catch a removed lock, and it is worse than it sounds because **A22 is a golden that every comment edit moves and a human hand-clears — twice in SE.5 alone — so A22 firing ALONE is indistinguishable from ordinary churn.** `regexp_replace(p.prosrc, ''--[^\n]*'', '''', ''g'')` deletes each line comment and keeps its newline, so the anchor still spans the statement''s two lines and an interleaved comment can no longer break it either; with it, the same decoy reds at 93 / 2. **F159 files the CLASS, not this instance:** any pgTAP cell asserting over `prosrc` is asserting over the file''s PROSE as well as its code unless it strips comments first');

select is(
  (select count(*)::int from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'leagues_scoring_reference_guard'
      and regexp_replace(p.prosrc, '--[^\n]*', '', 'g')
          ~ 'NEW\.scoring_system_id\s+FOR (UPDATE|KEY SHARE)(\s|;)'),
  0, 'K2 (ANCHORED 2026-08-26 by SE.5 — F155; it was unanchored and it FIRED ON A COMMENT): …and not a lock at either extreme. `FOR UPDATE` conflicts with the FK''s own `FOR KEY SHARE` and would serialise league writes against the FK path itself; `FOR KEY SHARE` does NOT conflict with the rules-UPDATE this seam races and would read as protection while providing none. **`FOR SHARE` was deliberately kept OUT of this ban list by F147 so that SE.5''s decision would not be a fight with a green pin — and SE.5 then took it** (D273; migration 105 §5): it is the weakest mode that still conflicts with the `FOR NO KEY UPDATE` every UPDATE of the referenced row takes, so the seam stays shut in both orderings against a no-lock control that BREACHES in both, while the mode it replaces made `create_league` serialise globally on six template rows — swept over CONCURRENCY (W workers x 20 calls on one template, mean ms/call, two reps), `FOR NO KEY UPDATE` rises monotonically 0.60 → 2.96 ms across W = 1..10 while `FOR SHARE` stays flat at ~0.7 (R647 relabelled an earlier form of this figure that read as six per-worker samples at a fixed W=6 when it was the sorted drain cascade at one level). **The argument is the CONFLICT TABLE, not the strength ordering:** `FOR SHARE` conflicts with `FOR NO KEY UPDATE` and `FOR UPDATE`, exactly the locks an UPDATE and a DELETE of the resolved row take, so every WRITER is excluded; what the stronger mode additionally excluded was other wall-3 invocations, which are READERS and cannot invalidate each other''s premise. The ban list is unchanged and still correct: what it bans is the two extremes, and the middle is now occupied. **The REGEX changed, and the reason is F149''s lesson arriving one cell late:** unanchored, this cell matched the words `FOR UPDATE` ANYWHERE in the body, so 105''s comment recording that SE.5''s RPCs take `leagues` FOR UPDATE before touching `scoring_systems` — a lock-ORDER note, on a different table, in prose — turned it RED. F149 anchored K1 and did not look at its neighbour. **THE ANCHOR ALONE DID NOT MAKE THAT TRUE, AND THE FIRST FORM OF THIS SENTENCE OVERCLAIMED TWICE (corrected 2026-08-26, F159).** (a) It said the cell *"measures the lock on the reference and not the file''s vocabulary"* while still matching RAW `prosrc` — measured: re-add the lock-order note in the anchored form (`NEW.scoring_system_id FOR UPDATE`) and this cell reds again with the real lock untouched and correct (93 total, **2 not-ok** — A22 + K2), the mirror of the false NEGATIVE the same defect gave K1. It is true now, and the thing that makes it true is that the match runs over `prosrc` **with line comments stripped** (`regexp_replace(p.prosrc, ''--[^\n]*'', '''', ''g'')`), the same strip K1 takes. (b) It said *"anchored to the same statement K1 anchors to"*, which it is not and should not be: K1 requires the whole `WHERE s.id = ...` clause, this cell requires only the reference expression followed by the lock word — strictly less. **The asymmetry is deliberate and it is the right way round:** a PERMIT should be as narrow as the statement it permits, a BAN should be wider than it, so a banned mode attached to `NEW.scoring_system_id` is caught anywhere in the code even if a later edit moves it out of that exact `WHERE`');
