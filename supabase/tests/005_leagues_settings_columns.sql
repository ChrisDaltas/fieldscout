...
select columns_are('public', 'leagues',
  array['id', 'owner_id', 'name', 'description', 'max_teams',
        ...
        'creation_action_id', 'avatar_url', 'champion_team_id'],
  ...
  -- updated by 118: +champion_team_id (§11.5 "Champion recorded on complete" — Q39 (D), L.D1.8)
  'leagues = the 12 001 columns + the 14 §12.1 columns + 055''s invite_slug + 060''s creation_action_id + 064''s avatar_url + 118''s champion_team_id, exactly');
...
--    landed in 052; 054 dropped "League owners can manage" per the Q8
--    ruling — league state is server-authoritative, owner SELECT rides the
--    swap policy's owner branch). 006/008 own the behavioral tests.
-- ---------------------------------------------------------------------------
select policies_are('public', 'leagues',
  array['Leagues viewable by members'],
  'leagues carries ONLY the member/owner SELECT policy (054/Q8: no client write path)');
