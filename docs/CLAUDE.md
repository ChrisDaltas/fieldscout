...
4. **Free users: 2 position submissions per week.** A submission is a locked snapshot of a ranked list for a given week or season. Count submissions per position for the current week before allowing new ones.
...
8. **Lists are ordered by default.** The `hide_order` field defaults to FALSE. All lists have a meaningful player order. Only set `hide_order = TRUE` when the user explicitly marks a list as unranked. The Big Board can never have `hide_order = TRUE`.

9. **Player order = order added, always.** Players always go to the bottom when added to a list — no auto-sorting, no smart insertion. The Smart Order button lets the user sort the full list by fantasy points, projected points, consensus rank, or ADP at any time. When a user commits a custom order via "Set as My Rank" or drag-and-drop, store that order in `overall_rank` on `list_players`.

10. **Submissions are immutable.** Once a user submits a list snapshot for a week or season, that submission record cannot be edited or deleted. The source list remains editable.

11. **Soft deletes.** Never hard-delete lists, teams, or submissions. Set `deleted_at` to current timestamp.
