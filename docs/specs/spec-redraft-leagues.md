...
|---|---|---|---|
| ~~`profiles.display_name`~~ | — | **RETIRED (v2.9 ruling).** FieldScout stores no name for a person: an account is an email, a password and a username. The column still exists but is never written and never read *(migration 075 stops `handle_new_user` seeding it from the OAuth `full_name` claim and clears the values already stored)*. **Precisely:** no name is stored in `public.profiles`, which is the anon-readable surface. An OAuth provider's own `full_name` claim still lands in `auth.users.raw_user_meta_data` — not anon-readable, never rendered, and ignored by the trigger — so "we store no name" is a statement about the data anyone can reach, not about the auth provider's payload | **Nobody — there is nothing to see** |
