...
- **Launch:** any league member starts a mock from a pre-draft league (`setup`/`scheduled`) — "Practice drafting here." The mock snapshots the league's **real draft config** (type, pick clock, order incl. their actual slot, rounds from roster slots, anti-snipe) into a new `drafts` row with `is_mock = TRUE`. The human takes their real seat by default (any seat selectable); every other seat is a CPU. *(Erratum v2.16 — **UNFILLED SEATS BECOME CPUs; A MOCK NEVER REFUSES TO START FOR WANT OF HUMANS.** D103(1) required the league's full seat map to exist before a mock could launch, on the correct reasoning that `draft_picks.team_id` is `NOT NULL` and the order needs every seat — and the result was a live refusal, hit in real use: *"practice draft failed, it says i need 11 more people."* The constraint is satisfied rather than relaxed: the launch mints a **mock seat** per empty slot — a `teams` row with `league_id IS NULL`, owned by the launcher, recorded in `config.mock.cpu_seats` and deleted with the mock — so the seats exist without any write to the real league (a placeholder seat is a real `teams` + `league_members` insert and is therefore forbidden here by this section's own zero-side-effects rule). A league with **one** seated franchise practises against eleven CPUs. Only a league with **no** active franchise at all still refuses, with a message that names its own reason. Task lane `docs/specs/tasks-MP-mock-practice.md` (MP.1); PROGRESS D227.)*
...
- **v1 scope:** ~~league-attached only (config, roster shape, and the player pool come from the league). A standalone practice lobby with quick settings~~ — **AMENDED v2.16, see the block below: a STANDALONE practice mock is v1.** A league-attached mock keeps everything this section describes, and its inheritance of the league's settings is a feature rather than a constraint. **Multi-human mock rooms remain v1.1** — that half of this bullet is unamended, and it is a statement about a *solo* practice's authority model (§8.8's v2.15 block), not about where a mock's settings come from.
...
**A mock draft is for practicing — a league is optional context, not a prerequisite (NEW v2.16 — Chris, 2026-08-21).** *"also keep in mind a user may run a mock draft without even being 'in a league'. i'm not sure why you are so focused on making a mock draft connected to a specific leagues settings."* · *"the main use case is just 'I want to practice drafting these players at these rounds or cost to see where my team ends up at the end'. But they may not have an actual league they are in yet or their friends league probably hasn't decided on everything yet."*

- **THE PREMISE THIS AMENDS.** Since v2.3 this section has described a mock as a **rehearsal of one specific league** — *"solo practice under the league's real settings"*, launched *"from a pre-draft league"*, with a v1 scope of *"league-attached only"* — and every mechanism follows honestly from it. **Practice is the purpose; a league is optional context that makes practice better.** The two statements are not in tension: a league-attached mock keeps everything it has, and a second kind of mock exists that never had a league. The reason the old premise held is the workflow it assumed: **a friends league settles its settings by group text and enters them "when they are all in the draft lobby together"**, so the moment someone most wants to practise is *before* any league on the platform has the settings in it. The premise assumed the league arrives first; in the real workflow it arrives last.
- **TWO ENTRY POINTS, and each answers a different question.** **(1) From the FieldScout Home tab** — *"launch a mock based on public league settings which will be standardized"* — a **standalone** mock that owns its own settings, taken from one named **standardized preset**. **(2) From a League Home, beside the real draft-lobby button, "Practice drafting here"** — *"this one inheriting your league settings is actually a really awesome feature that current platforms fail to do effectively."* **Inheritance is the feature, not a limitation**, and nothing overrides it (see the clock bullet below).
- **A STANDALONE MOCK'S SETTINGS ARE ITS OWN, AND ITS ISOLATION OBLIGATION IS PROVEN VACUOUS RATHER THAN SKIPPED.** A standalone practice is backed by a hidden, user-owned **practice container** that no league surface ever enumerates: it holds no other members, no rosters, no matchups and no invites, and it exists only to give a practice somewhere to live. **This section's zero-side-effects rule is unweakened and its meaning is sharpened, not broadened** — it protects the **real league** a mock is attached to, and a standalone mock simply has no such league. That vacuity is established by measurement (a whole-schema delta across the practice's entire lifecycle), never asserted, and a league-attached mock's isolation proof is unchanged in every particular.
- **THE STANDARDIZED PRESET IS A DEFAULT, AND IT IS WHERE THE MOCK BID CLOCK LIVES.** The preset is the settings schema's own defaults plus the default scoring template, authored once and mirrored, adjustable and expected to change with feedback. **A standalone mock launches with a 10-second auction bid clock** — the number ruled for mocks — **as a default filling a value nothing else supplies. It overrides nothing.** A league-launched mock keeps its league's clock, so this section's fidelity promise — *"A mock is slow when the league's clocks are slow, and that is correct"* — **stands unamended**, and a launcher who wants a faster clock than their league chose **changes it in the room** (§8.7's v2.15 mock carve-out). *(This resolves PROGRESS **Q23** with no override and closes AP.4 with no code; it does not by itself meet this section's under-45-minute bar — the remaining term is PROGRESS **F92**.)*
- **THE MOCK DRAFT REPORT, AND WHERE IT LIVES.** *"it would also be great to have a Mock Draft Report that lives somewhere, that is basically a table of every player from the draft and their cost and what team they went to."* A completed mock's recap gains a **flat table** cut alongside the board and the rosters: **one table with two column sets, never one table with blanks** — an auction report is *player · position · team · **price** · **nomination #***, a snake/linear report is *player · position · team · **round** · **pick #***, because an empty column claims a value exists and is unknown, which is false in both directions. **Mock reports live on the USER, not on the league** (*"mocks should go to something on the User's profile, not at the league level. However, real drafts should have a place at the league level"*) — a user-scoped surface listing every past mock, reachable from a **Mock Drafts** entry in the app's **More…** section, with each row opening its report. **The league-level recap keeps real drafts and is otherwise unchanged**, and the split is a route move with a redirect, never a deletion — existing mock-recap links keep working.
- **MOCKS ARE INDEPENDENT OF LEAGUE WORK (RULED — Chris, 2026-08-21: *"Mocks do not need to be connected to League work"*).** The practice surfaces carry **their own release gate**, never the leagues gate: turning leagues off must not turn practice off, and no practice task may be blocked on a league task. **This is a RELEASE statement, not a code-isolation one** — a mock still runs on the shared draft engine (`drafts`, `draft_tick`, the room, the same RPC family, the same suites) and that sharing is correct and stays. **Independence is a claim about ROUTES as well as about components:** a practice surface that lives under a league-gated route path is gated whatever its own flag says, so the practice room, its list and its report carry a route family of their own. The one surface that legitimately depends on leagues is the League Home CTA, because it sits on a league page. The gate covers **UI surfaces only** — never a `SECURITY DEFINER` RPC, whose authorization stays in-body and server-authoritative (§12).
- **Task lane:** `docs/specs/tasks-MP-mock-practice.md` (prefix `MP.`, ten tasks); PROGRESS **D225–D232**, **F102–F106**, **Q24**, **E78–E80**.
...
| E78 | *(v2.16)* A 12-team league with **one** seated franchise launches a mock; the same league fully seated launches another | **Both launch.** The under-seated one drafts against **eleven CPU seats** — `teams` rows with `league_id IS NULL`, owned by the launcher, listed in `config.mock.cpu_seats` — and the fully-seated one produces a `draft_order` **byte-identical to what it produced before this change** (same league, same mock id). Around both lifecycles (launch → picks → delete) the real league's whole `leagues` row, its `teams` and `league_members` rows and its whole real `drafts` row are **byte-identical** (whole-row composites, never counts). Only a league with **no** active franchise still refuses, naming its own reason. **A mock never refuses to start for want of humans.** |
| E79 | *(v2.16)* A user who belongs to **no league at all** launches a mock from the Home tab, with the leagues release gate **OFF** and the mock gate **ON** | **It launches, drafts, completes, and its report opens** — Home entry, room, report and the More… entry all render. The mock takes the **standardized preset** (including the 10-second auction bid clock, as a default). A whole-schema delta across the whole lifecycle shows changes confined to the practice container, the mock's own draft rows and its `cpu_seats` teams: **no `league_rosters`, no `transactions`, no notification, no league-status transition, and no broadcast on any `league:<id>` topic but the container's own.** The container never appears in the user's league list — a user with one sees the identical `/api/leagues` payload as a user without one, **and its id never appears in a URL: the practice room and its report are keyed on the MOCK's id.** **The room half of this row is a requirement ON THE MOCK'S OWN ROUTE FAMILY, not on `/app/leagues`** — the leagues release gate hard-redirects every `/app/leagues` URL *"even by direct URL"*, so a mock room living there could not satisfy this row and the row is not satisfied until the practice room has a route of its own (tasks-MP **MP.10**). |
| E80 | *(v2.16)* An auction mock's report and a snake mock's report are opened side by side | **Two column sets, one component.** Auction: *player · position · team · **price** · **nomination #*** with per-team spend. Snake/linear: *player · position · team · **round** · **pick #***. **Neither prints a column belonging to the other**, blank or otherwise. A league-attached mock and a standalone mock render the **identical** report at the **identical** route; a stale league-scoped recap URL carrying `?draft=<mock_id>` **redirects** to it rather than 404ing, and the league recap for a **real** draft is unchanged. |
...
## Changelog
- **v2.16 (2026-08-21):** **A MOCK DRAFT IS FOR PRACTICING — the premise that a mock is a rehearsal of one specific league is wrong, and is amended (Chris, in-session 2026-08-21; folded by the Architect per the fold-back rule — PR open for approval).** **MINOR bump, not a patch:** this entry amends a v1-scope statement the LAW made and changes what a mock *is*. **Companion breakdown `docs/specs/tasks-MP-mock-practice.md` (prefix `MP.`, ten tasks); PROGRESS D225–D232, F102–F106, Q24, E78–E80.** **(1) THE PREMISE (§8.8's new v2.16 block, and its amended v1-scope bullet).** *"a user may run a mock draft without even being 'in a league'. i'm not sure why you are so focused on making a mock draft connected to a specific leagues settings."* **Practice is the purpose; a league is optional context that makes practice better.** A **standalone** mock — settings its own, taken from one standardized preset, backed by a hidden user-owned practice container no league surface enumerates — is **v1**. Multi-human mock rooms remain v1.1, unamended. **(2) UNFILLED SEATS BECOME CPUs (§8.8 Launch erratum).** D103(1) required the league's full seat map before a mock could launch, and that produced a live refusal in real use — *"practice draft failed, it says i need 11 more people."* **A mock never refuses to start for want of humans.** The `NOT NULL` constraint that motivated the gate is **satisfied rather than relaxed**: a mock seat is a `teams` row with `league_id IS NULL`, owned by the launcher, listed in `config.mock.cpu_seats` and deleted with the mock — so the seats exist with **no write to the real league** (a placeholder seat writes `teams` *and* `league_members` and is forbidden here by this section's own rule). Isolation by construction, not by an added exclusion. **(3) TWO ENTRY POINTS.** Home — a standalone mock on the standardized preset; and League Home, beside the real draft-lobby button, **"Practice drafting here"** — *"this one inheriting your league settings is actually a really awesome feature that current platforms fail to do effectively."* **Inheritance is the feature.** **(4) THE MOCK DRAFT REPORT, ON THE USER RATHER THAN THE LEAGUE.** *"a table of every player from the draft and their cost and what team they went to"* — a flat table with **two column sets, never one table with blanks** (auction: price + nomination #; snake: round + pick #), reachable from a **Mock Drafts** entry under **More…**. *"mocks should go to something on the User's profile, not at the league level. However, real drafts should have a place at the league level"* — so the league recap keeps real drafts unchanged and mock links **redirect** rather than break. Whether the user-scoped surface is private app chrome or a public profile tab is **Q24**, deliberately not decided here (recommendation: private). **(5) MOCKS ARE INDEPENDENT OF LEAGUE WORK (ruled: *"Mocks do not need to be connected to League work"*).** The practice surfaces carry their **own release gate** — turning leagues off must not turn practice off. **This is a release statement, not a code-isolation one:** a mock still runs on the shared draft engine and that sharing is correct and stays; and the gate covers UI surfaces only, never a `SECURITY DEFINER` RPC. **(6) Q23 IS ANSWERED WITH NO OVERRIDE, and §8.8's fidelity sentence is UNTOUCHED.** Because inheritance is the feature, *"A mock is slow when the league's clocks are slow, and that is correct"* **stands exactly as written**; the 10-second mock bid clock ruled by Q17 lives in the **standardized preset as a default** for a mock that has no league to inherit from, and overrides nobody. The in-room remedy for a league that chose a slower clock is §8.7's v2.15 mock carve-out. **AP.4 closes with no code and PROGRESS B5 clears; F85 is NOT discharged — its remaining term is F92.** **What this entry does NOT weaken: §8.8's zero-side-effects rule** (unchanged, and now stated over both kinds of mock, with a standalone mock's obligation **proven vacuous rather than skipped**), **§8.7 in any particular** (that is v2.15's fold), and **the v2.15 launcher-is-the-commissioner block**, which is orthogonal and intact. §19.2 gains **E78–E80**.
