...
- **Known refusals at the time of this entry (four, not one).** **`draft_reset`** writes `leagues.status` and strips the stored auto-start instant — the live-proven breach v2.8.16 was written for; whether a mock-safe variant replaces it is task MS.4's decision, and if built it must return the mock to **`live` at pick 1** (a `scheduled` mock cannot be restarted — `draft_start_internal` excludes mocks). **The autopick toggle** (§8.7's "Toggle autopick for any team") writes `league_members.is_autodraft` on the real seat by construction, and a mock never reads that flag anyway. **Seat reassignment** reaches the league's real invite/assignment surface. **The "Edit draft order" control is a fourth and a different shape: it is MIS-TARGETED rather than blocked** — its request carries no draft id, so the service resolves the league's *real* active draft and the RPC's own mock refusal is never reached; excluding it therefore has to happen in the service, not in the refusal message. None of the four gets a mock-scoped shadow of a league table — §8.8's v1 scope is league-attached on purpose. **The general rule this establishes: a control is proven mock-safe only when it has been driven through the surface a user actually reaches it by, with a real draft alive in the same league** — an RPC-level measurement alone cannot see a mis-target.
...
- **Task lane:** `docs/specs/tasks-MS-mock-sandbox.md` (prefix `MS.`); PROGRESS **D216–D221**, **E75/E76**.
