...
| E81 | *(v2.16.7)* A commissioner edits the league's custom scoring while a draft room reading that league's scoring document is open in the same browser | **The room reads the new document, and it does not wait for a cache to expire.** Both §15.1 scoring writes invalidate the league's scoring query (`['league-scoring-family', <league_id>]`, `staleTime` 10 minutes) together with the league detail the fork's repoint changes — so a client that made the edit never renders pre-edit scoring, and never renders the *"Scoring not readable"* degraded state because of its own successful save. A **refused** write invalidates nothing (there is nothing new to read). This is bounded by design and the bound is stated rather than implied: another member's already-open room is a different cache and is bounded by the same `staleTime`, and no REAL draft is exposed at all — both writes refuse outside `setup`/`scheduled` (§7.3 header) and a started draft scores from the frozen snapshot. |
