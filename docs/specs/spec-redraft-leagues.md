...
**Status: DRAFT — this block awaits Chris's markup (the #228 gate precedent: the carrying PR stays open until he marks it up; nothing here authorizes a build).** Task **SC.4** (ledger **F202**) is takeable only after the markup folds and the PR merges. The marks to resolve, enumerated: the **rename** (below), the wizard's **opening side** (§7.3.3's system-default bullet), and Scout PPR's **two `[DRAFT COPY — Chris to edit]` marks** (the card one-liner in §7.3.3's table, and the copy below).
