...
| E79 | *(v2.16)* A user who belongs to **no league at all** launches a mock from the Home tab, with the leagues release gate **OFF** and the mock gate **ON** | **It launches, drafts, completes, and its report opens** — Home entry, room, report and the More… entry all render. The mock takes the **standardized preset** (including the 10-second auction bid clock, as a default). A whole-schema delta across the whole lifecycle shows changes confined to the practice container, the mock's own draft rows and its `cpu_seats` teams: **no `league_rosters`, no `transactions`, no notification, no league-status transition, and no broadcast on any `league:<id>` topic but the container's own.** The container never appears in the user's league list — a user with one sees the identical `/api/leagues` payload as a user without one. |
