...
| `qb` | QB | QB | 1 |
| `rb` | RB | RB | 2 |
| `wr` | WR | WR | 3 *(v2.16.9 DRAFT — was 2; the Scout default roster, Chris 2026-08-26, pending his markup)* |
| `te` | TE | TE | 1 |
...
- **The 8 templates** *(v2.7: v1 ships the **6 parity templates only**; the two FieldScout templates return when advanced stats are funded)* *(v2.16.9 — DRAFT, pending Chris's markup: **Scout Scoring** joins as the **seventh shipped template and the system default for new leagues** — the row below, the system-default bullet after the table, and the full stat-by-stat table with its basis and assumptions in **Appendix B.5**)* (system-owned `scoring_systems` rows, `is_template = TRUE`, `owner_id NULL`; full point tables in **Appendix B**):
  | Template | One-liner |
  |---|---|
  | **Scout Scoring** *(v2.16.9 DRAFT — FieldScout's default, pending Chris's markup)* | *One clean rule set.* Every TD 6 — passing TDs included — every FG 3, no PPR, 0.1/yd rush+rec · 0.05/yd pass, −2 all turnovers. Full table + basis: **Appendix B.5** |
...
Reuses the existing `scoring_systems.rules JSONB` (see `docs/03-DATA-MODEL.md`). Ship the v1 templates as system rows (`is_template = TRUE`, `owner_id NULL`) — **v2.7: the 6 parity templates at v1; the B.2–B.3 FieldScout rows wait for funded advanced stats.** *(v2.16.9 — DRAFT, pending Chris's markup: **B.5** adds **Scout Scoring**, FieldScout's house template and the system default for new leagues — box-score keys only, no advanced-stat dependency, so it ships with the parity six rather than waiting with Alpha/Ultra.)* Decimal supported throughout. **Platform values below are best-known as of July 2026 — re-verify every cell against ESPN/Yahoo/Sleeper official help pages at build time** (⚠ marks the cells most prone to drift: K distances, D/ST tiers).
...
> **v1 UI is a template picker** (§7.3.3), not an editor: **6 parity cards at v1** (v2.7 — Alpha/Ultra cards and the "same game, scored three ways" widget return when advanced stats are funded) with side-by-side compare. The full editor (this catalog exposed per-category) returns in v1.1. *(v2.11: the 2026 test cohort gets the §7.3.3.1 editor over the `scoring_surface: 'scorable'` subset of this catalog — bonuses, TE premium, and IDP stay reserved.)* *(v2.16.9 DRAFT: the picker becomes 7 cards, Scout Scoring first and preselected — see B.5 and §7.3.3's system-default bullet.)*

### B.5 Scout Scoring — *FieldScout's default* (NEW v2.16.9 — DRAFT, pending Chris's markup)

**Status of every value in this section:** it is either **RULED** (Chris's verbatim 2026-08-26 ruling below — LAW for this draft), **inherited** from the six shipped templates with the basis stated (unanimity or majority, read from the shipped `templates.ts` bodies and migration 058's seeds — never from memory of what a platform does), or an explicit **`[ASSUMPTION — Chris to confirm]`**. The PR that adds this section **stays open until Chris marks it up** (the #161/#152 approval-gate precedent) — nothing here authorizes a build. **Name history, cited rather than hidden:** *"Scout Scoring"* was minted once before and **withdrawn** (PROGRESS D229(2), 2026-08-21 — no ruled values existed, and Chris's phrase then was about a public-league product). This entry re-mints it because the values now exist; the name itself is his to change at markup.
...
| `pass_2pt` | 2 | `[ASSUMPTION — Chris to confirm]` — "all TDs 6" does not cover 2-pt conversions; all six shipped rows pay 2 (unanimity inherit) |
| `rush_yards` | **0.1** | RULED — "10 yards to 1pt" (matches all six) |
| `rush_tds` | **6** | RULED (all six already 6) |
| `rush_2pt` | 2 | `[ASSUMPTION — Chris to confirm]` — unanimity inherit |
...
| `rec_2pt` | 2 | `[ASSUMPTION — Chris to confirm]` — unanimity inherit |
| `fumbles_lost` | **−2** | RULED — "−2 for all turnovers" (matches all six). Only a fumble **lost** scores: the registry has no plain-fumble key, so lost-only is structural — `[ASSUMPTION — Chris to confirm]` that this matches intent |
...
| `pat_made` | 1 | `[ASSUMPTION — Chris to confirm]` — XP unstated; all six pay 1 (unanimity inherit) |
| `fg_missed` | 0 (key omitted) | `[ASSUMPTION — Chris to confirm]` — **the one value where this draft does NOT take the incumbent majority.** Four rows pay −1 (ESPN ×2 verified, Sleeper ×2 best-known/F24); Yahoo ×2 omit. Drafted at no-penalty for consistency with the flat-FG philosophy (distance luck isn't skill, and most misses are long attempts the coach ordered); flip to −1 if the majority is wanted |
| `pat_missed` | 0 (key omitted) | `[ASSUMPTION — Chris to confirm]` — majority omit (ESPN and Yahoo carry no PAT-miss penalty; Sleeper −1 best-known/F24) |
| `def_sack` | 1 | `[ASSUMPTION — Chris to confirm]` — D/ST unstated; identical across all six (unanimity inherit) |
| `def_int` | 2 | `[ASSUMPTION — Chris to confirm]` — unanimity inherit. **The passing-INT / D/ST-INT distinction, said out loud:** `interceptions` −2 is charged to the QB who threw it; `def_int` +2 is your D/ST's takeaway. "−2 for all turnovers" is read as penalizing giveaways, never takeaways |
| `def_fumble_rec` | 2 | `[ASSUMPTION — Chris to confirm]` — unanimity inherit (same giveaway/takeaway reading) |
| `def_td` | **6** | "All TDs 6" **plausibly covers D/ST TDs** — and all six incumbents already pay 6, so the ruling and the inheritance agree either way. `[ASSUMPTION — Chris to confirm]` only as to whether his sentence meant to reach it |
| `def_safety` | 2 | `[ASSUMPTION — Chris to confirm]` — unanimity inherit |
| `def_block` | 2 | `[ASSUMPTION — Chris to confirm]` — unanimity inherit |
| `def_return_td` | **6** | "All TDs 6" plausibly covers it; all six already 6 (same reading as `def_td`) |
| `def_pa_0` / `def_pa_1_6` / `def_pa_7_13` / `def_pa_14_20` / `def_pa_21_27` / `def_pa_28_34` / `def_pa_35_plus` | 10 / 7 / 4 / 1 / 0 / −1 / −4 | `[ASSUMPTION — Chris to confirm]` — points-allowed unstated. **Majority pick, flagged:** four of six rows (Yahoo ×2, Sleeper ×2) use the shared single-model family at these values (Yahoo-verified; Sleeper best-known/F24); the ESPN ×2 minority uses the split PA + YA model on ESPN's own buckets. Scout takes the shared family, `dst_model = 'single'`, **no yards-allowed table**. Carrying the full 7-key family (never just the 4 shared names) also keeps the doc unambiguous to `scoring_detect_tier_cuts` — pgTAP 053 §B's family detection then covers the 7th template automatically |
| `qb_sack_taken` | — (omitted) | unscored by all six (unanimity inherit); it stays a B.3/Ultra idea |

**Decimal scoring: on** — RULED, and already §7.3.3's default; every value above is a 2dp multiple, so the §7.3.3.1 guardrail-5 rule (|coef| ≤ 100 at 2dp — SE.3's validator, SE.4's SQL mirror) holds untouched. As drafted the rules object carries **32 coefficients** (14 offense/turnover/return + 4 kicking + 7 D/ST events + 7 PA tiers; `return_yards` excluded until F71).

**The roster half — a SEPARATE surface, deliberately.** Starts **1 QB / 2 RB / 3 WR / 1 TE / 1 FLEX (W/R/T) / 1 K / 1 D/ST** (10 starters), **bench 6**, **1 IR spot** *(type unrestricted, OUT/IR designations — `[ASSUMPTION — Chris to confirm]`: he said "1 IR spot"; the type and designations inherit the shipped default)*, Hot Swap off. These are `leagues.roster_settings` defaults (§7.3.2's preset table, amended in this version), **not scoring rules: the §7.3.3.1 format-2 envelope has no roster fields and must not grow any** (SE.4's envelope-shape sweep already refuses unknown members — 23 envelope shapes measured). Measured delta from today's shipped default: **exactly one number, WR 2 → 3** (`DEFAULT_ROSTER_SETTINGS` at `src/lib/leagues/settings/league-settings.ts:164`, migration 040's column DEFAULT, and §7.3.2's table already match every other value). It lands as its own task (SC.2) at §7.3.2's landing sites — never inside the scoring document.
