...
- **Product shape (Chris's four requirements).** **(1) Scoring is configured BY POSITION** — the editor steps through positions (QB → RB → WR → TE, plus K/D-ST per Q12), each position entering its scoring; not the industry's global category style. **(2) An "All Positions" switch per section** applies that section's values to every position; per-position customization stays possible afterward. **(3) Every position except K and D/ST carries five sections:** Rushing · Passing · Receiving · Special Teams (e.g. return yards — Q13) · Turnovers (fumbles, interceptions). **(4) A live example player per position** whose fixed sample stat line recomputes as values are edited; sample players are FIXED — **Dan Marino (QB), Randy Moss (WR), Adrian Peterson (RB), Gronk (TE)**.
...
- **F21 guardrails (validation LAW, not UI hope — what a format-2 doc structurally cannot express):**
  ...
  2. **Bucketization exclusivity (the F21 double-pay itself):** collected doc-wide, `def_pa_*` keys must be a subset of **exactly one** named platform set (shared 7-key XOR ESPN 8-key — the sets overlap in 4 keys, so subset-of-one is the check, per the §23.5 registry's set definitions ≡ `derive-stats.ts` boundaries); `def_ya_*` ⊆ the single 9-key YA set. Mixing sets (e.g. `def_pa_14_20` + `def_pa_18_27`, which one-hot together at PA 18–20) is rejected.
...
- **Sample players & lines (fixed in spec so the UI is deterministic; recomputed through the REAL pipeline — `resolveRules` + `scorePlayerWeek`, never a parallel math path; pinned by hand-computed literals per the D62 discipline).** Lines contain only delivered stats (no pending badge inside the editor, by construction); context numbers (carries, attempts) render as flavor and score nothing:
  ...
  Each position page shows its player's line with the live total under the doc-as-edited; the numbers above are product voice — Chris may amend them on this entry's PR without re-opening the mechanism. K/D-ST sample lines: Q12.
...
src/components/leagues/scoring-editor.tsx           §7.3.3.1 custom scoring editor (test cohort): position stepper, per-section value grids, All-Positions switch (derived state), live sample-player line per position via the REAL calculator
src/components/leagues/scoring-editor-ops.ts        pure ops layer: resolveRules, section/position key catalogs, switch-state derivation, normal-form strip — mirrored by the server-side guardrail validation (§7.3.3.1)
...
- **Registry additions for the custom editor (v2.11, §7.3.3.1):** `STAT_KEYS` gains **(a)** a `scoring_surface` field — `'scorable'` (editor-exposed; the core_box column/derived keys templates draw on) vs `'context'` (raw sources `def_points_allowed`/`def_yards_allowed` + aggregates `fg_made`/`fg_attempted`/`pat_attempted`/`pass_attempts`/`pass_completions`/`rush_attempts`/`targets` — never scorable in a format-2 doc, validation-rejected) vs `'reserved'` (deferred bonuses, placeholders, IDP) — and **(b)** the named **bucket-set definitions** for the derived families (shared-PA set, ESPN-PA set, YA set — the same boundaries `derive-stats.ts` owns), which the §7.3.3.1 exclusivity guardrail validates against. `return_yards` is a candidate new key via the checklist below (Q13 — no column/mapping exists today).
