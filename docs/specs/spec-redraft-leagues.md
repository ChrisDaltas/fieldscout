...
- **The 8 templates** *(v2.7: v1 ships the **6 parity templates only**; the two FieldScout templates return when advanced stats are funded)* *(v2.16.9, marked up by Chris 2026-08-31: **Scout Scoring** joins as the **seventh shipped template and the system default for new leagues** — the row below, the system-default bullet after the table, and the full stat-by-stat table with its basis and assumptions in **Appendix B.5**)* *(v2.16.11 DRAFT — pending Chris's markup: Scout becomes a **PAIR** — **Scout Standard** and **Scout PPR**, identical bodies one value apart (`receptions` 0 vs **0.2**), per his 2026-08-31/09-01 pair ruling — the two rows below, the amended system-default bullet, and Appendix B.5.1)* (system-owned `scoring_systems` rows, `is_template = TRUE`, `owner_id NULL`; full point tables in **Appendix B**):
  | Template | One-liner |
  |---|---|
  | **Scout Standard** *(v2.16.9 — FieldScout's default, seeded as "Scout Scoring"; marked up by Chris 2026-08-31. v2.16.11 DRAFT: the no-PPR half of the Scout pair — `[ASSUMPTION — Chris to confirm: rename "Scout Scoring" → "Scout Standard"]`, App B.5.1)* | *One clean rule set.* Every TD 6 — passing TDs included — every FG 3, no PPR, 0.1/yd rush+rec · 0.05/yd pass, −2 all turnovers. Full table + basis: **Appendix B.5** |
  | **Scout PPR** *(NEW v2.16.11 DRAFT — pending Chris's markup; App B.5.1)* | Scout Standard's rule set, one value apart: **0.2 per reception**. `[DRAFT COPY — Chris to edit]` *The catch counts — volume alone still can't outscore production.* |
...
- **System default (NEW v2.16.9 — marked up by Chris 2026-08-31):** a new league's scoring step opens with **Scout Scoring preselected** — the commissioner keeps it, picks an incumbent template, or Customizes (§7.3.3.1) — and the standalone mock launcher preselects it the same way (§8.8). Today NEITHER surface preselects anything (measured: `league-create-wizard-ops.ts:67` and `mock-launch-ops.ts:103` both initialize `scoringSystemId: null`, and both submit gates refuse a null). The default is a **preselection, never a silent write**: `create_league` still receives an explicit template id, and D170's template-only birth predicate is untouched. The picker leads with the Scout card (this table's order is the picker order) and carries Appendix B.5's "why it's better" copy. *(v2.16.11 DRAFT — the pair amendment, pending Chris's markup: the SYSTEM default stays **Scout Standard** — no-PPR is the FieldScout identity his original ruling named — and under the wizard's reception-style filter **each family preselects its own Scout**: the No-PPR view preselects Scout Standard, the PPR view Scout PPR, and the unfiltered mock launcher keeps preselecting the system default. Three consequences, said out loud: **(1)** D282(3)'s opening-side flip DISSOLVES — the style step no longer hides a default whichever side it opens on; both views carry their own. **(2)** The SC.3 review's swap-hazard finding RETIRES — a style flip's reset-refill now lands on the active family's own Scout, the intended default for that view, never a surprise substitution (numbering note: PROGRESS D284(3) — the R-numbers this finding was briefed under were taken mid-task by #238's SC.2-residue filing). **(3)** The step's opening side becomes a pure identity choice with no mechanical consequence — the one residual decision, his: `[ASSUMPTION — Chris to confirm: opens No PPR, the FieldScout identity]`.)*
...
src/components/leagues/scoring-template-picker.tsx  template cards + compare (v2.7: 6 parity cards at v1; "same game, scored three ways" widget returns with Alpha/Ultra; v2.11: gains the "Customize" fork affordance — §7.3.3.1; v2.16.9, marked up 2026-08-31: a seventh card — Scout Scoring, listed first and preselected as the system default, carrying App B.5's "why it's better" copy; v2.16.11 DRAFT: an eighth card — Scout PPR beside Scout Standard, each style-filtered view led and preselected by its own Scout — App B.5.1)
...
Reuses the existing `scoring_systems.rules JSONB` (see `docs/03-DATA-MODEL.md`). Ship the v1 templates as system rows (`is_template = TRUE`, `owner_id NULL`) — **v2.7: the 6 parity templates at v1; the B.2–B.3 FieldScout rows wait for funded advanced stats.** *(v2.16.9, marked up by Chris 2026-08-31: **B.5** adds **Scout Scoring**, FieldScout's house template and the system default for new leagues — box-score keys only, no advanced-stat dependency, so it ships with the parity six rather than waiting with Alpha/Ultra.)* *(v2.16.11 DRAFT: B.5 becomes the Scout **pair** — Scout Standard + Scout PPR, one value apart; B.5.1.)* Decimal supported throughout. **Platform values below are best-known as of July 2026 — re-verify every cell against ESPN/Yahoo/Sleeper official help pages at build time** (⚠ marks the cells most prone to drift: K distances, D/ST tiers).
...
| `receptions` | **0** | RULED — "No PPR", a **stated difference** (key present at an explicit 0, the Standard-row convention). *(v2.16.11 DRAFT: the **Scout Standard** value — the pair's ONE divergence; **Scout PPR** carries `receptions: 0.2` — B.5.1)* |
...
**Why it's better — user-facing copy — APPROVED as drafted (Chris, 2026-08-31)** *(the #228 markup fold's blanket approval covered this copy — D278(8)(c), "the user-facing copy" — but the fold flipped every B.5 mark EXCEPT this heading's `[DRAFT COPY — Chris to edit]`; the leftover mark is corrected in passing, v2.16.11 — measured `grep -c "DRAFT COPY" docs/specs/spec-redraft-leagues.md` → 1 before the fix, this heading at spec:2386)* (surfaces on the Scout picker card, §16.2):
...
#### B.5.1 The Scout pair — Scout Standard + Scout PPR (NEW v2.16.11 — DRAFT, PENDING CHRIS'S MARKUP)

**Status: DRAFT — this block awaits Chris's markup (the #228 gate precedent: the carrying PR — #239 — stays open until he marks it up; nothing here authorizes a build).** Task **SC.4** (ledger **F203**) is takeable only after the markup folds and the PR merges. The marks to resolve, enumerated: the **rename** (below), the wizard's **opening side** (§7.3.3's system-default bullet), and Scout PPR's **two `[DRAFT COPY — Chris to edit]` marks** (the card one-liner in §7.3.3's table, and the copy below).
...
*(The card one-liner is drafted in §7.3.3's table row under the same mark. Return yards stay absent from this copy too — the copy never promises a gated stat.)*
