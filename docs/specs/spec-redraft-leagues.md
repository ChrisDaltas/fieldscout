...
- **Create league** (free — v2.8 ruling; no `is_pro` gate): name, season, `team_count` ∈ {8, 10, 12, 14, 16} (v1 — see changelog v2.6), and a settings wizard (7.3). Creator becomes `commissioner` and gets a team.
  - **CREATION SEATS THE WHOLE BOARD (NEW v2.14 — Chris, 2026-08-20, from hitting the alternative live).** Creating an *N*-team league creates ***N* franchises in the same transaction**: one owned by the creator, the other *N*−1 as **unclaimed placeholder seats**. A league is a board of *N* franchises from the instant it exists; it is never a board of one that the commissioner has to finish assembling by hand.
  - **Why this is a correctness change and not a convenience.** `team_count` is the number the commissioner *chose*; a league that says "12 teams" and contains one is already lying about itself, and every downstream surface then has to cope with a shape the settings say is impossible. What Chris hit: a brand-new 12-team league refused **Run mock draft** with *"league has 1 of 12 franchises seated"* and told him to go create eleven placeholder seats before he could rehearse — a chore invented by the data model, presented as a requirement of the product. The same refusal guards `draft_start` (§7.2's Capacity bullet, D96), so the chore was owed twice.
  - **Invites CLAIM a seat; they no longer create one.** The join/claim paths seat a user onto an existing unclaimed franchise (the seat-targeted flow already worked exactly this way — §12.23). This is what makes seat-targeted and open-link invites one mechanism instead of two, and it is why the growth-loop funnel (§16.4) can count seats claimed against seats that exist.
  - **The capacity refusals STAY, and must not be softened.** `draft_start`'s and `create_mock_draft`'s "*n* of *N* franchises seated" checks are unchanged in force and wording; they simply become unreachable through the ordinary path, which is the correct relationship between a guard and a fixed defect. They remain reachable — and must remain pinned — for a league whose franchise count has been changed after creation (retirement, an edited `team_count`).
  - **Explicitly REJECTED: letting the mock create the seats it needs.** It is the smaller-looking fix and it is the wrong one — §8.8's **zero-league-write** isolation ("no `league_rosters`, no `transactions`, no league status transitions") is a verified, pinned property, and a mock that writes `teams` rows into the league to make itself runnable trades a proven guarantee for a convenience. The seats must exist before the mock asks, which is what creation now guarantees.
...
- **Join** with invite code (free — v2.8: creation is free as well, so no Pro gate exists anywhere on the create/join surface). ~~On join, a `teams` row is created for the manager with `league_id` set.~~ **v2.14: on join the manager CLAIMS an unclaimed franchise** (creation made them all); the joiner may rename it. A league with no unclaimed seat left refuses the join by name rather than growing an *N*+1th team — the seat count is the commissioner's setting, and a join has never been allowed to change it.
- **Placeholder/managed teams.** Commissioner can create empty seats (a `teams` row owned by the commissioner, flagged `is_placeholder`) so the draft can run before everyone has joined; ownership can be reassigned to a real user later (audited). *(v2.14: creation already makes all *N* of them, so this is no longer a step anyone must take to reach a startable league — it stays as the repair affordance for a board that has since lost a franchise, and as the mechanism the create path itself uses. The "add an open seat" call-to-action disappears from the ordinary setup flow along with the state that needed it.)*
...
| `draft_scheduled_at` | — | timestamptz | ~~When the room opens.~~ **v2.14: INFORMATIONAL ONLY** — the time the league agreed to meet. It drives the countdown and every "when is our draft" display and it **starts nothing**; the commissioner presses Start (§8.3.1, §8.5.1). |
...
- Auction: `auction_budget ≥ roster_size × reserve`, where `reserve` = **1** with `auction_zero_dollar_nominations` OFF and **0** with it ON (every team can fill a legal roster). *(v2.13 — this bullet used to read `× auction_min_bid`; the field is gone and the reserve is now derived from the toggle. With the toggle ON the bullet is satisfied by every legal budget, which is the point: at $0 nominations nothing can price a team out of a roster spot.)*
...
### 8.3.1 The draft schedule is INFORMATIONAL (NEW v2.14 — Chris, 2026-08-20)

*"just an FYI the 'draft schedule' the date and time is just for people to get organized it functionally should do nothing."*

- **Kept, unchanged:** `draft_scheduled_at` as a setting; the countdown on league home and in the lobby; the league-reference/viewer-local rendering (§16.4's timezone rule); the `scheduled` status and everything that keys on it. A draft time is how ten friends agree on a Thursday, and that is a real job.
- **Removed:** the draft **starting itself** at that instant. A draft begins exactly one way — **the commissioner presses Start** (§8.5.1, §16.5.1). The `draft-tick` auto-start scan is retired (§8.2's erratum), and with it the client-side watch that polled for the flip.
- **Removed with it, and this is the point of the change:** every piece of copy that promises an automatic start. A countdown that says *"the draft starts automatically at the scheduled time"* is a promise the product will now break, and a broken promise about draft night is worse than no promise.
- **Why the auto-start existed, and why losing it is the right trade.** It was the answer to "what happens when the scheduled hour arrives and three managers never show up" — the room would start regardless and their seats would autodraft (§8.5.5's away path, E48's no-user seats). That machinery is untouched and still does its job **once the draft is live**; what changes is that the draft does not become live without a person deciding it should. The failure it prevents is a league whose draft ran without them; the failure it caused is a league whose draft ran without them. Chris chose the side where a human decides, and the commissioner already had the button.
- **What must NOT be weakened by this change:** the `draft_start` capacity refusal (§7.2/D96), the scoring-snapshot-before-transition ordering (§7.3.3), the create-if-absent behaviour of the ONE start implementation, and every §8.5.5/§8.6.2 disconnect and timeout contract. This removes a *trigger*, not a code path — `draft_start` itself is unchanged, and the commissioner's button has always called it.
...
1. ~~Room opens at `draft_scheduled_at` (or commissioner clicks **Start Draft**).~~ **v2.14: the commissioner clicks Start Draft.** Status → `drafting`. The scheduled instant is informational (§8.3.1) — the lobby is reachable before it, the countdown runs to it, and nothing starts at it.
...
- ~~**"My Lists" panel** in the draft room (a tab beside *My Queue*)~~ **v2.14 — the lists are a FILTER, not a place.** §16.4's dock correction retires the Lists tab: the list picker lives **beside the position chips in the Players region**, and it still offers every list the user attached to this league, any **league-shared** lists, and their season Big Board by default. Chris: *"lists were supposed to be Filters within the 'Players' tab."* Nothing in this section's capabilities is dropped — each one below says where it now lives.
- **Cheat sheet:** open any attached list (ranked, with tiers) in a side panel while drafting. *(v2.14 — the list's per-player **notes** no longer live only here: they render **in the player's own row** in the pool and the auction table, so the note is in front of you at the moment the player comes up. See the §16.4 note rule.)*
- **Overlay on the pool:** toggle a list as an overlay on Available Players — show each player's rank/tier from that list as a column, and/or **filter the pool to "only players on this list."** *(v2.14 — this IS the Lists filter: selecting a list in the Players region's filter row is the overlay, and "Only this list" is that control's narrowing on-state. One mechanism, now named as what it always was.)*
- **Load into queue:** one tap to **load an attached list into the draft queue** in list order (skipping already-drafted players), or "Add remaining" to append; reorder afterward as normal. *(v2.14 — **this action lives in the TARGETS region**, not with the lists. Chris, 2026-08-20: *"a user should be able to load a List into the Targets. so like before a draft they can make their list of players to target and use that in case they will be on auto draft"* — and, on the shipped placement, *"i don't see anyway to do it in the UI right now. I figured from the targets tab i should be able to select a list."* **The reason it must not be buried:** this is the **autodraft insurance path**. §8.4's resolution chain reads Targets FIRST, so loading a prepared list into Targets is how a manager who cannot attend still drafts their own board instead of falling through to ADP. A capability that only works if you find it is not insurance. Open Targets, pick a list, it loads. **Precondition to carry with it:** the load reads an ATTACHED list, so a picker offering an unattached list (the Big Board included) must attach it as part of the same gesture rather than 404.)*
...
- ~~Mobile: the panel is a bottom sheet.~~ *(v2.12 replaced the sheet with the dock; v2.14 replaces the panel with the filter. Below the dock cutoff the filter and its picker are the same controls in the tabbed Players region — the list surfaces are one implementation at every width.)*
- **Attach / detach / set-primary** stay reachable from the filter's own list picker (§7.4's affordances), and **"best available from my board"** moves with them — it belongs beside the board that produced it. *(v2.14: recorded here so no capability is lost with the panel; §16.2 carries the same map component-by-component.)*
...
> **The draft room is DARK, and the draft room ONLY (NEW v2.14 — Chris, 2026-08-20).** Asked whether the dark treatment he was looking at should spread, he ruled: *"yeah just this page definitely not the whole app. don't ever assume things like that."* So the room — the `(room)` route group, i.e. the lobby and the live room, and nothing else — renders on a dark neutral palette. The **recap stays light**, because it stays inside the app shell (the rule above); every other league surface is untouched.
>
> **This is an explicit carve-out from `CLAUDE.md`'s "Single theme … never generate dual-theme tokens", and the carve-out is narrow on purpose.** That rule exists to prevent a **user-facing light/dark toggle** and the doubled token set a toggle requires — two values for every colour, a preference to store, and every screen owing both. The room has none of that: it is dark for **everyone, always**, with **no toggle, no preference, and no `dark:` variant** (the project sets no `darkMode` key, so Tailwind's default `media` strategy would make `dark:` follow the viewer's OS setting — precisely the behaviour this is not). It is one route rendering on a different neutral palette, the way the app's own ink sidebar already renders dark inside a light page.
>
> **What changes and what does not.** The dark treatment re-points the **neutral layer only** — page, surface, hairline/border, primary text, muted text. The product's **identity tokens are unchanged in the room**: `accent` (ultramarine, "do a thing"), `brand` (lime, "look here"), `positive`/`negative`/`caution`, the `pos-*` position identities and the `tier-*` ramp all keep their app-wide values, because a position badge that means one colour on the Lists page and another in the draft room is two products. A design reference may show a different selection colour; the room does not adopt it.
>
> **The room reuses the shared components — it does not fork them.** Button, Badge, Card, `PlayerRow`, `PositionBadge` and the rest are the same components the light app mounts (CLAUDE.md's no-near-duplicate rule is not suspended for the room). They must therefore go dark **without a per-component change**, which makes the mechanism part of the contract rather than an implementation detail: the neutral tokens resolve through a **scoped variable indirection** that the room's frame overrides, so a shared component the room mounts *for the first time next month* is dark by construction rather than by somebody remembering. A per-component `variant="dark"` fork is explicitly rejected — it multiplies every shared surface by two and fails silently for anything added later. (Mechanics, the equivalence obligation, and the sweep for components that hardcode a colour instead of consuming a token: PROGRESS **D205**, tasks-DR2 DR2.2.)
...
src/components/draft/draft-dock.tsx                 v2.14 — ONE slide-up panel holding THREE SIMULTANEOUS REGIONS (Players · Targets · Roster) above `screen-dock`, and the same three as tabs below it; non-modal, Esc closes. (v2.12 shipped five sibling tabs incl. Lists and Chat — superseded: Lists is a filter inside Players, Chat is a pop-out)
src/components/draft/draft-chat-popout.tsx          v2.14 — the chat pop-out (§16.4): closed by default, launcher in the room's chrome, quiet unread indicator that distinguishes a commissioner SYSTEM post from chatter (§16.3); hosts `draft-chat.tsx` unchanged
...
src/components/draft/available-players.tsx          searchable/filterable pool w/ Big Board/ADP/tier + league-list overlays; v2.14 — the list overlay becomes a FILTER CONTROL beside the position chips (the "only players on my list" mode is that control's on-state), and a filtered row carries its `list_players.notes` note inline (§16.4)
src/components/draft/my-queue.tsx                    drag-to-reorder queue — LABELED "Targets" in all UI (v2.10 naming rule, §8.4); v2.14 — owns the "load a list into my Targets" action (§8.9), which moved here from the retired Lists panel because it is the AUTODRAFT INSURANCE path (§8.4's resolve chain), not a convenience
src/components/draft/my-lists-panel.tsx              league-tagged lists: cheat sheet (incl. per-player notes — v2.10), pool overlay, load-into-queue, primary board. **v2.14 — no longer a dock tab.** Its duties split: overlay/only-mode → the Players filter · load-into-Targets → `my-queue` · attach/detach/set-primary → the list picker the filter opens · cheat-sheet notes → the player row (§16.4)
src/components/draft/my-roster-tracker.tsx          slots filling up + needs
src/components/draft/auction-block.tsx              current nomination + high bid + time remaining (prominent), bid input, budgets, max-bid, anti-snipe; team columns per §16.4's v2.10 auction-room-layout callout; v2.13 — carries the 3-second uncontestable-award beat (§8.6.9), a client-held presentation delay, never a server one; v2.14 — those columns are the BOARD ZONE's own content (real columns filling the zone's height, never a row of landscape cards), and they never vanish at any width (§16.3's overflow rule)
src/components/draft/auction-player-table.tsx       expandable table of available players w/ projections, filters, column customization, row actions (v2.10 — §16.4); v2.14 — carries the same in-row list note `available-players` does
src/components/draft/draft-chat.tsx                  realtime chat + system (commish) posts — v2.14: hosted by `draft-chat-popout`, not by the dock
...
- **Scroll, don't rearrange (NEW v2.14 — Chris, 2026-08-20: *"allow each section to be scrollable horizontally within itself"*).** When a room surface is wider than the space it has, it **scrolls horizontally inside its own bounds**. It does not shed columns, collapse into a summary, or hide behind a "show all" button. The room is the same room at 1280 as at 1600 — you scroll more or less — and that sameness is what makes a layout feel stable to someone who has been staring at it for two hours. Applies to every dock region and to the auction's team columns. Two consequences worth stating: a surface's overflow is **its own** (one region scrolling never moves another), and a horizontal scroll is the **overflow** behaviour, not the everyday one — a sensibly-sized set of twelve team columns should mostly fit on a laptop, and if it never fits the columns are too wide.
- **Working surfaces you compare are simultaneous; working surfaces you visit are not (NEW v2.14).** Mid-draft you read your roster *while* scanning the pool and *while* checking your Targets — so those three are on screen together (§16.4). Chat is the one surface you want open *while doing something else*, which is exactly why it is not one of them: it is a pop-out that runs alongside. The test for anything new is which of those two sentences it satisfies.
...
- **Draft room layout (v2.12 — Chris, 2026-08-17; this callout is the layout contract, and it supersedes the room composition M2 shipped).** The room is a full-screen, chrome-free page (§16.1) laid out in **three zones, top to bottom**:
  ...
  Everything else lives in a **bottom dock** that **slides up over the board and back down as ONE unit**. Default state is **closed** (the focused board view is the point); the open dock **overlays** the board rather than resizing it — the board's geometry must not move when the dock opens; Escape closes it; the room behind stays live and operable (§16.3).

  **THE DOCK'S COMPOSITION — CORRECTED v2.14 (Chris, 2026-08-20), and this supersedes v2.12's five-sibling-tabs reading BY NAME.** v2.12 printed the dock's contents as "Available players · Targets · Roster · Lists · Chat … one panel open at a time", and **DR.5 built exactly that** — five sibling tabs where opening one *replaces* the others. Chris used it and ruled that reading wrong: *"no the dock doesn't go away it actually goes up and down in unison and those sections are supposed to be tabs within the dock."* The error was not the builder's care, it was the spec's sentence: "one panel at a time" is a correct description of a *drawer*, and it was applied to surfaces a manager needs **at the same time** — choosing Roster hid the entire player pool, so mid-draft you could see what you had or what was left, never both. That is the substance of *"the pieces are all here but it's super confusing."* What the dock actually is:

  - **Three regions, side by side, ALL VISIBLE AT ONCE: Players · Targets · Roster.** No tabs among them. *"Players and Targets should be able to live side by side"* and, on where Roster goes, *"you might want to see everything at once actually."* **Players is the wide one** — it carries a dozen stat columns, the search, the position chips and the toggles; Targets and Roster are lists of player names and are naturally narrow rails. (Proportion recommendation, not a ruling: ~55% / ~22.5% / ~22.5%. The design reference splits its dock 967px / 753px at 1720 — 56.2% to the table — and halving the right side gives two ~376px rails.)
  - **Lists is NOT a region.** *"lists were supposed to be Filters within the 'Players' tab."* §8.9's list overlay and its "Only this list" mode already **are** that mechanism; they become a filter control beside the position chips, and the Lists tab is removed (§16.2 records where the panel's other duties land).
  - **Chat is a POP-OUT, not a region.** *"Chat should be it's own pop out."* It is the one surface you want open *while doing something else*, and it is also the least important — *"Chat is the least important tho."* So it is **closed by default**, launched from the room's own chrome, and carries a **quiet unread indicator** rather than a presence that must be dismissed. The least important thing in the room must not cost a decision on entry. One obligation survives the move intact: §16.3's rule that a commissioner action **shows in chat so the room sees it happen** — so the indicator must distinguish a **system post** from chatter, because a commissioner action is a room event and a closed pop-out must still announce one.
  - **Overflow is horizontal, per region** (§16.3's scroll-don't-rearrange rule). The player table sheds no columns; no rail collapses.

  **Widths — three tiers, and the cutoff is ONE named token (v2.14).**

  | Width | The dock |
  |---|---|
  | **≥ the cutoff** | Players · Targets · Roster side by side, each scrolling horizontally within itself |
  | **< the cutoff** | The same three as **tabs**, one at a time |
  | Phone | Tabs, full width — as today |

  Two rulings ride this table. **(1) The cutoff is a single named breakpoint token defined once** (a custom screen in `tailwind.config.ts`), consumed by every region, by the tab strip and by the auction's team columns — **never Tailwind's stock `lg:` scattered across components.** Chris asked whether the cutoff *"can be bumped up to 1280 later"*, and the answer is only "yes, one line" if it is one named value. **The room being replaced is the proof:** `auction-block.tsx`'s team columns are governed by a bare `hidden … lg:flex`, which is why twelve team columns vanish entirely on a 13-inch laptop while the rest of the room keeps working — one hardcoded breakpoint in one component produced a feature that disappears at a width nobody chose. **(2) The below-cutoff tabs treatment is INTERIM.** Chris: *"i think i'll eventually do dedicated tablet and mobile layouts."* It exists so the room is usable below the cutoff, not to be good: keep it minimal, give it no narrow-only features, and structure the composition so a purpose-built narrow layout can be dropped in without the wide one caring — the opposite of how the room being replaced got its mobile treatment, which was the desktop structure handed down. **The silver lining, stated so the correction is not read as a discard:** DR.5's shipped tab machinery is not thrown away — it stops being the desktop layout and becomes the narrow-width layout, where tabs are genuinely the right pattern.

  *(Open, and deliberately not decided here: the dock's open HEIGHT — the fixed `min(60vh, 640px)` v2.12 shipped, or a draggable one. The design reference gives its dock 698.6px of a 998px frame, i.e. ~70%. PROGRESS **Q19**.)*
- **Mobile draft room density (v2.0; RECONCILED with the dock at v2.12).** At 20 teams the pick grid is 20 columns, so the board zone on mobile still collapses to a **ticker + "my picks" rail with the full grid one tap away** — that rule stands and is unchanged. What changes is *where the other surfaces live*: the M2 room's four-way mobile pane switcher (Players / Queue / Full board / Chat) is **replaced by the same bottom dock desktop uses**, so there is now ONE pattern on both platforms instead of two competing ones. Mobile specifics: the dock's tab strip scrolls horizontally if the labels do not fit, a panel opens taller than on desktop (the board behind it is a ticker, not a grid), and the app's own global bottom navigation is **absent** in the room (the room is outside the app shell), so the dock has the bottom edge to itself — no stacked bars. *(v2.14: phone keeps the tabs treatment, now with THREE tabs rather than five — Lists is a filter and Chat is a pop-out at every width. This whole bullet describes the **interim** narrow path; purpose-built tablet and phone layouts are intended and will replace it — tasks-DR2 §8.)*
- **Prep notes travel to the moment they are needed (NEW v2.14 — Chris, 2026-08-20).** A list's per-player note (`list_players.notes`, v2.10 — the ruled use case is auction cost prep, *"$8 max"*) renders **in the player's own row** wherever the room shows that player: the pool, the auction table, and the cheat sheet it already shipped in. Asked whether that was the right home, Chris: *"yes in the players row itself is perfect."* The reason is timing — a note you wrote a week ago is worth nothing in a panel you would have to go and open; it is worth everything on the row of the player currently up for bid. Two rules carry over unchanged: the note comes from the **list currently filtering the pool** (a note is a property of a list, so an unfiltered pool shows none — a row cannot invent which list's opinion to quote), and a player with **no note renders nothing at all** — no empty affordance, no placeholder chrome, on any row (§16.5.4's designed-empty rule is about copy, not blank containers).
...
- **Auction room layout (v2.10 — Chris, 2026-08-15):** every team gets its own column (or row on mobile), ordered by **nomination order**, showing: drafted players with prices · which roster positions remain undrafted · total spots left to fill · total remaining budget · current max bid. At-a-glance visual identification of: **my** team · the team **up for nomination** · the team that placed the **latest bid**. The current nominated player, current high bid, and time remaining stay prominently visible (the `auction-block` centerpiece).
  - **The team columns ARE the auction's board zone (CORRECTED v2.14 — Chris, 2026-08-20).** A snake room fills zone 3 with the rounds × teams pick grid; an auction has no such grid, so zone 3 is the nomination centrepiece plus **the team columns, laid out as actual columns that take the zone's height** — the thing the v2.10 word "column" always meant. **What shipped instead is a row of fixed-width landscape cards inside a horizontal scroller**, and the consequences were measured live at 1440×900 on a 12-team league: the columns need **2082px inside a 1407px container**, so **four of twelve teams are always off-screen**, while content ends at y=631 of 900 and leaves **~270px of the zone empty**. A surface that scrolls sideways past a third of the league while a quarter of its own zone is blank is using the wrong axis. The correction: the columns occupy the zone as columns, **all twelve visible without horizontal scrolling at a normal laptop width**, using the vertical room the zone already has. Horizontal scroll remains the **overflow** behaviour under §16.3's rule — and under that same rule **there is no width at which a team column disappears**, which supersedes the shipped `lg:`-gated treatment that hides every column below 1024px behind a "Show all teams" button.
...
|---|---|---|---|
| `setup` | **Setup checklist**: settings ✓ · seats *n*/*N* (each **unclaimed** seat → invite affordance) · scoring template chosen · schedule draft | Invite managers / Schedule draft | Practice-draft card once a draft is configured. *(v2.14: all *N* franchises exist from creation — §7.2 — so this row counts **claimed** seats against a board that is already whole. There is no "add a seat" chore and no state in which the checklist asks for one.)* |
| `scheduled` | Draft countdown (league TZ + local) — **informational; nothing starts at zero (v2.14, §8.3.1)** | **Enter draft lobby** · Practice this draft (§8.8) | Order reveal (if manual/reveal), template card, checklist remainder. The lobby's **Start draft now** is the commissioner's one trigger, reachable before, at and after the scheduled instant; no surface may promise an automatic start |
...
- **Banners:** "Live stats delayed" (§23.2) · "Charting lands Mon AM" (Ultra, §23.5) · realtime-fallback "reconnecting — scores refresh every 10s" (§9) · deploy-freeze/maintenance (admin-set) · acting-as (commissioner operating an orphaned team).
  - **The uncontestable award (v2.13, §8.6.9)** is a **transient room message on the auction block**, not a banner and not a chat line: *"No one can bid. Awarding {Player} to {Team}."*, held for exactly **3 seconds** on every client, then the board paints the award. It is a presentation beat over an award the server has already committed — the room must never gate an action on it, never let a reconnect during those 3 seconds lose the award, and never show it twice (§16.3's say-a-thing-once rule). A client that joined mid-beat simply sees the completed award, which is correct.
...
- **Chat pop-out indicator (NEW v2.14, §16.4):** the room's chat launcher carries a **quiet unread mark** — never a count that demands zeroing, never an auto-open. It has **two states, not one**: ordinary unread chatter, and an unread **commissioner system post**, which is louder because §16.3 requires the room to see a commissioner action happen and a closed pop-out is the only place that rule can now be broken. Opening the pop-out clears it; nothing else does.
...
| E67 | *(v2.13)* A team nominates a player at $1 when **every other** active franchise has `max_bid = 0` (rosters full, or budgets exhausted) | §8.6.9: the nomination and the award land in **one transaction** — a `draft_picks` row at the opening price, the same solvency re-check, the rotation advanced — and no bid clock is ever opened. The room shows *"No one can bid. Awarding {Player} to {Team}."* for exactly 3 seconds, identically on every client, then paints the board. **Negative control:** one other team with `max_bid ≥ high_bid + 1` (including a team whose whole budget is $1) makes the nomination ordinary — the bid clock opens and the award waits for it. |
...
| E71 | *(v2.14)* `draft_scheduled_at` passes with the league in `scheduled` and nobody in the lobby | **Nothing happens.** The league stays `scheduled`, no `drafts` row is created or started, the countdown reads as elapsed, and no surface claims a start is imminent (§8.3.1). The commissioner's **Start draft now** is the only trigger and works identically before, at and after the instant. **Negative control:** the same league with the commissioner pressing Start — `draft_start` runs unchanged, capacity and snapshot ordering enforced exactly as before. |
| E72 | *(v2.14)* A brand-new 12-team league, seconds old, launches a mock draft | Succeeds. Creation seated all 12 franchises in the create transaction (§7.2), so `create_mock_draft`'s "*n* of *N* franchises seated" refusal is not reached and the commissioner is never sent away to build seats. **The refusal itself still exists and still fires** for a league whose franchise count later diverges (a retired franchise, an edited `team_count`) — pinned by constructing that state directly, not by regressing creation. |
| E73 | *(v2.14)* The dock is open at a width where the player table's stat columns exceed the Players region | The **Players region scrolls horizontally inside itself** (§16.3). Targets and Roster do not move, no stat column is dropped, no rail collapses, and the page itself never scrolls horizontally. Same rule and same expectation for the auction's team columns, where all franchises stay reachable at every width — there is no width at which a team column is absent from the DOM. |
| E74 | *(v2.14)* A commissioner pauses the draft while a manager has the chat pop-out closed | The pause is announced in the command bar as always (§16.5.4), **and** the chat launcher's indicator enters its **system-post** state — §16.3's "the room sees it happen" is preserved through a closed pop-out. Opening the pop-out clears the indicator and shows the system post in place; nothing else clears it, and the pop-out never opens itself. |
...
## Changelog
- **v2.14 (2026-08-20):** **DRAFT ROOM v2 — the dock's real shape, a dark room, the auction's board zone, an informational schedule, and a league that is a whole board from creation (Chris, in-session 2026-08-20 after driving the finished room on the dev stack; folded by the Architect per the fold-back rule — PR open for approval).** Companion breakdown `docs/specs/tasks-DR2-draft-room-v2.md` (prefix `DR2.`); design reference is the Figma frame `Wol5Ssp7z0UMZckRVDKJi7` node `108:2742`, which is a **snake** room and contains no auction furniture — it supplies the room's shape and visual language, and the auction's own content is derived, never copied. **(1) §16.1 — the room is DARK, and the room only** (*"yeah just this page definitely not the whole app. don't ever assume things like that."*): an explicit, narrow carve-out from CLAUDE.md's single-theme rule, which exists to forbid a user-facing **toggle** and a doubled token set — the room has neither, and must not use `dark:` variants (the project sets no `darkMode` key, so `dark:` would follow the viewer's OS). Only the **neutral layer** re-points; `accent`/`brand`/`pos-*`/`tier-*` keep their app-wide values. Shared components are reused, not forked, so the mechanism is a **scoped variable indirection** the room's frame overrides — a component the room mounts later is dark by construction (D205). **(2) §16.4 — THE DOCK'S COMPOSITION IS CORRECTED, and v2.12's five-sibling-tabs reading is superseded BY NAME.** v2.12 wrote "one panel open at a time" and DR.5 built exactly that; Chris ruled the reading wrong (*"no the dock doesn't go away it actually goes up and down in unison"*). The dock slides as **ONE unit** holding **three simultaneous regions — Players · Targets · Roster** (*"Players and Targets should be able to live side by side"*; *"you might want to see everything at once actually"*), with **Lists demoted to a FILTER inside Players** (*"lists were supposed to be Filters within the 'Players' tab"* — §8.9's overlay and its "only this list" mode already were that mechanism) and **Chat promoted out to a POP-OUT** (*"Chat should be it's own pop out"*, *"Chat is the least important tho"* ⇒ closed by default, quiet unread indicator, with a **system-post state** so §16.3's "the room sees a commissioner action happen" survives a closed pop-out). The dock's own error is named rather than glossed: choosing Roster hid the whole pool, which is the substance of *"the pieces are all here but it's super confusing."* **(3) §16.3 — two new principles: *scroll, don't rearrange*** (*"allow each section to be scrollable horizontally within itself"* — no shed columns, no collapsed rails, the same room at 1280 and 1600) **and the simultaneous-vs-visited test** that puts Players/Targets/Roster together and Chat alongside. **(4) §16.4 — three width tiers with the cutoff as ONE NAMED TOKEN.** Side-by-side above it, the same three as tabs below it, tabs on phone. The token is a single custom screen consumed by every region, the strip and the team columns — never a stock `lg:` scattered across components, because Chris asked whether the cutoff *"can be bumped up to 1280 later"* and that is a one-line answer only if it is one value. The room being replaced is the proof: a bare `lg:` on the auction team columns is why twelve columns vanish on a 13-inch laptop. The below-cutoff treatment is explicitly **INTERIM** (*"i think i'll eventually do dedicated tablet and mobile layouts"*), and DR.5's tab machinery is re-scoped to it rather than discarded. **(5) §16.4 — the auction's board zone is team COLUMNS.** Measured on the shipped room at 1440×900: 12 columns need 2082px in a 1407px container (four teams always hidden) while ~270px of the zone sits empty — the wrong axis. The columns fill the zone as columns, all visible at a normal laptop width, and under (3) no width hides one. **(6) §16.4 — per-player list notes render in the PLAYER'S ROW** (*"yes in the players row itself is perfect."*): the note comes from the list currently filtering the pool, and a player without one renders nothing. **(7) §8.3.1 (NEW) + §8.2/§8.5.1/§7.3.8/§16.5.1 — the draft schedule is INFORMATIONAL** (*"just an FYI the 'draft schedule' the date and time is just for people to get organized it functionally should do nothing."*): the date, countdown and displays stay; the **auto-start is removed** — the `draft-tick` D94 scan, the two client watch-polls, and every piece of copy promising an automatic start. The commissioner presses Start. The rationale for the old behaviour is recorded, not erased (it was the answer to three managers not showing up — machinery that still works once the draft is live). **(8) §7.2 — creating an *N*-team league creates *N* franchises**, one the creator's and *N*−1 unclaimed; invites **claim** a seat instead of creating one. Chris hit the alternative live: a new 12-team league refused a mock with *"league has 1 of 12 franchises seated"* and sent him to build placeholder seats. The capacity refusals are unchanged in force and stay pinned for the states that can still reach them; **letting the mock create its own seats is explicitly rejected** — it would trade §8.8's verified zero-league-write isolation for a convenience. **(9) §19.2 — E71–E74** (elapsed schedule does nothing · a seconds-old league mocks successfully · per-region horizontal overflow · a system post through a closed pop-out). **Numbering:** this entry takes **v2.14** above the auction-pacing **v2.13**, whose PR (#178) was open when this fold began and **merged mid-session** (`e9e699c`, 2026-08-20) — first-filed keeps its number (the R312 principle, D144(6)). This branch was then **rebased onto it and the five overlapping sites resolved deliberately** rather than mechanically: the version header, §16.2's `auction-block` / `auction-player-table` / `draft-chat` lines (both lanes' clauses kept — v2.13's 3-second beat and v2.14's board-zone correction describe different things about the same component), §16.4's auction-player-table callout (v2.13's Q16 ruling kept intact, with v2.14's board-zone sub-bullet added above it), the §19.2 table (E67–E70 then E71–E74) and this changelog. Edge cases **E67–E70 are v2.13's**; this entry starts at E71 for the same reason. **Open and deliberately not decided here:** the dock's open height (**Q19**), whether an auction re-uses the Roster region given its team column already shows spots and buys (**Q20**), and the cutoff's number (**Q21** — 1024 recommended).
