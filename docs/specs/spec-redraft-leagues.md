...
|---|---|---|---|
| ~~`profiles.display_name`~~ | — | **RETIRED (v2.9 ruling).** FieldScout stores no name for a person: an account is an email, a password and a username. The column still exists but is never written and never read *(migration 075 stops `handle_new_user` seeding it from the OAuth `full_name` claim and clears the values already stored)* | **Nobody — there is nothing to see** |
