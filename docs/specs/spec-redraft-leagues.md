...
- **A MOCK IS STANDALONE. FULL STOP.** No league, no inherited settings, no hidden container standing in for one. **Its settings come from the six base scoring templates** (§7.3.3 / App B — ESPN Standard & Full PPR, Sleeper Standard & Full PPR, Yahoo Standard & Half PPR, already shipped) **plus this spec's own draft defaults**, and **the launcher changes them at launch**: *"just let the user make the clocks what they want. Use the same default … Everyone is going change the clock times so the defaults are merely a suggestion."*
