...
- **The 8 templates** *(v2.7: v1 ships the **6 parity templates only**; the two FieldScout templates return when advanced stats are funded)* *(v2.16.9, marked up by Chris 2026-08-31: **Scout Scoring** joins as the **seventh shipped template and the system default for new leagues** — the row below, the system-default bullet after the table, and the full stat-by-stat table with its basis and assumptions in **Appendix B.5**)* (system-owned `scoring_systems` rows, `is_template = TRUE`, `owner_id NULL`; full point tables in **Appendix B**):
  | Template | One-liner |
  |---|---|
  | **Scout Scoring** *(v2.16.9 — FieldScout's default; marked up by Chris 2026-08-31)* | *One clean rule set.* Every TD 6 — passing TDs included — every FG 3, no PPR, 0.1/yd rush+rec · 0.05/yd pass, −2 all turnovers. Full table + basis: **Appendix B.5** |
...
Reuses the existing `scoring_systems.rules JSONB` (see `docs/03-DATA-MODEL.md`). Ship the v1 templates as system rows (`is_template = TRUE`, `owner_id NULL`) — **v2.7: the 6 parity templates at v1; the B.2–B.3 FieldScout rows wait for funded advanced stats.** *(v2.16.9, marked up by Chris 2026-08-31: **B.5** adds **Scout Scoring**, FieldScout's house template and the system default for new leagues — box-score keys only, no advanced-stat dependency, so it ships with the parity six rather than waiting with Alpha/Ultra.)* Decimal supported throughout. **Platform values below are best-known as of July 2026 — re-verify every cell against ESPN/Yahoo/Sleeper official help pages at build time** (⚠ marks the cells most prone to drift: K distances, D/ST tiers).
...
| `receptions` | **0** | RULED — "No PPR", a **stated difference** (key present at an explicit 0, the Standard-row convention) |
...
**Why it's better — user-facing copy `[DRAFT COPY — Chris to edit]`** (surfaces on the Scout picker card, §16.2):
