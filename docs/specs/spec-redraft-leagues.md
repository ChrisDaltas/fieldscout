...
- Managers set starters before lock. **Lock** per `lineup_lock` setting (default per-player kickoff; alt first-game-of-week). Locked slots are read-only to managers. *(v2.16.16 — migration 112 `set_lineup`, measured: every lock instant is read from `nfl_games.kickoff_at` at evaluation time (E42/§23.3) and the slot is locked AT the kickoff instant (closed on the kickoff side, the E41 convention); `team_lineups.locked_at` is the RECORD of the instant evaluated at write time, never the decider. `per_player_kickoff`: a locked slot's player never moves (replace/move/bench refuse by name) and a player whose game has started never enters or changes slots, while every unlocked slot of the same lineup stays editable; a week with game rows but none for the player's team is a bye (no lock, flagged); a week with NO game rows falls back to the Mid-season-entry datum chain (`nfl_weeks.first_kickoff_at` → `starts_at`) for every player — conservative, the window only narrows. `first_game_of_week`: the whole lineup locks at the week's first kickoff (the same datum). The verb reads transaction time, never a caller-supplied instant. The submitted `slot_map` keys are the manager's placement; a per-slot-valid placement is honored verbatim and a per-slot-invalid one is re-seated along an augmenting path if any legal arrangement exists (E16 — the server returns the canonical map, naming the moves); otherwise the refusal names the player and the slot. The commissioner may set any team's lineup through the same verb under the same lock law, with the D290 interim audit posture (#253 fix round, R738): a reason is REQUIRED for an actor who is not the team's manager, a real change posts the D97 in-txn league-chat system message carrying it, the row records `edited_by_commish = TRUE`, and F40's control list grows with `set_lineup` (commissioner arm); the past-lock/retroactive edit below stays the audited override (M6), and until then the commissioner inherits the manager's `allow_illegal_lineups = FALSE` and E16 refusals (§11.1's "never block the commissioner" waits for M6 — F224(c)). A locked placement is a fact: a mid-week position relisting never moves a locked starter (R737). PROGRESS D308; pinned in pgTAP 060.)*
- **Auto-carry:** if a manager doesn't set a lineup, last week's legal lineup carries over (invalid/bye players flagged).
- **IR moves:** a player can be placed on an IR spot only while holding one of that spot's eligible designations; on placement set `ir_placed_week` (and, for **Restricted** IR, `ir_lock_until_week = ir_placed_week + min_weeks`). Moving a player **out of a Restricted IR** spot is blocked until `current_week ≥ ir_lock_until_week`; **Unrestricted** IR allows free in/out before lock. If an IR'd player loses eligibility, the roster is flagged illegal until they're moved off (Restricted stint still applies; commissioner can override). See §7.3.2 / §12.7. *(v2.16.16 — migration 112, measured: IR spots are roster-level state (`league_rosters`), so an IR move is judged against the CURRENT week whatever week the submit is for — under BOTH lock modes: `first_game_of_week` judges it against the current week's first kickoff, never the submitted week's (R736, the #253 blocker) — and `current_week` is the league's greatest `league_weeks` week whose `nfl_weeks.starts_at` ≤ now — the calendar table at the §23.3 boundaries `league-week-advance` flips on, never wall-clock arithmetic (until 114 lands, `league_weeks.status` has no writer). IR spots ride the canonical `slot_map` as `"<ir_key>:0"`; an IR key absent from a submitted map is a removal. The designation is read from `players.status` through a vocabulary bridge (the feed's `Out`/`Sus`/… → the catalog's OUT/Suspended/…; Questionable/Active/Inactive/NA are no designation). An IR move whose player's game has kicked off refuses on lock timing. PROGRESS D308; pgTAP 060.)*
