...
|---|---|---|---|
| `profiles.full_name` | Jason Jones | The account holder's real name. **Private** — editable in account settings, rendered on NO surface, in or out of a league *(v2.9 ruling; was `display_name`, which leaked OAuth real names everywhere)* | **No one but the account holder** |
...
League UI renders **Team name — @username**. The username is the **only** name the product displays anywhere; it is by definition the displayed name and we do not call it that (v2.9 ruling, 2026-08-05). Usernames are **permanent** (v2.8/Q4 ruling — no renames after selection; the auto-generated pre-selection placeholder is not a selection). League surfaces join on `user_id` regardless, and the audit log stores `user_id`. Email and `full_name` are the private fields. *(The v2.7 text made usernames changeable; superseded. The v2.1–v2.8 "display name (@username)" render rule is superseded by v2.9.)*
...
src/components/leagues/scoring-template-picker.tsx  template cards + compare (v2.7: 6 parity cards at v1; "same game, scored three ways" widget returns with Alpha/Ultra; full editor returns v1.1)
...
- **Invite conversion is the growth loop:** the join flow (invite link → signup → seated in league) must be measured step-by-step and owned like a funnel; a league that seats 10/10 managers is the product's best acquisition event. Seat-targeted invites (§7.2.1) get their own funnel cut (the "replacement GM" flow), and the pre-auth claim page shows league name, team name, and inviter — give people a reason to finish sign-up. **Identity display rule everywhere (v2.9):** *Team name — @username*; email and `full_name` never render in league surfaces.
...
> **v1 UI is a template picker** (§7.3.3), not an editor: **6 parity cards at v1** (v2.7 — Alpha/Ultra cards and the "same game, scored three ways" widget return when advanced stats are funded) with side-by-side compare. The full editor (this catalog exposed per-category) returns in v1.1.
...
## Changelog
- **v2.9 (2026-08-05):** **The identity ruling — the username is the only name FieldScout displays.** Ruled by Chris after seeing his own real name rendered across the app: `profiles` had no real-name field, and `handle_new_user` seeds the name column from the OAuth `full_name` claim, so signing in with Google put a REAL NAME into `display_name` — which every identity surface rendered as `display_name ?? @username`. **Resolution:** the column is renamed `profiles.display_name` → **`profiles.full_name`** (migration 075) and reclassified **private** — editable in account settings, labelled "Full name" with private-only helper copy, and rendered on no other surface, in or out of a league. Every identity render becomes the **@username** and nothing else: §7.2's identity table and render rule, §7.2.1's retained-history line, §12.23's claim-preview comment, and §16.4's growth-loop rule are updated in place; the RPCs follow (`create_league`/`seat_league_member_internal` derive `"<username>'s Team"`, `get_join_preview` returns the inviter's `@username`, `draft_actor_name` posts `@username`, `notify_list_followers` attributes `@username`). Seeding `full_name` from the OAuth claim is KEPT — now that the field is private, that seed is semantically correct. **The concept of a "display name" is retired entirely**, including from code identifiers: the username IS the displayed name and we do not call it that. AI personas are unaffected in substance — their name is PUBLIC brand copy carrying the "(AI)" transparency marker, so it keeps rendering, under the renamed `ai_personas.persona_name` (the phrase survives nowhere in the schema, and the one public name column is unmistakably distinct from the private `profiles.full_name`). F2 is untouched: the claim preview still exposes exactly three fields and never `invited_email`. Pinned in pgTAP 027 (columns + universal "no function reads display_name" + behavioural handle pins with a real `full_name` set on the actor) and `src/lib/identity-render.test.ts` (source-level sweep of every surface). *(Search also narrows to handle-only matching: matching on a private field would let anyone probe for a name they cannot see.)*
