...
| `draft_bids` | INSERT (one event per bid row — opening bids and raises) **and a per-STATEMENT void summary** (`operation: 'UPDATE'` — emitted once per `voided_at` stamp statement, never once per row: cancel/end = 1, a cascade undo = up to 2, a reset = 1 for the whole run; record = `{voided_at, voided_count, nominations:[{nomination_seq, player_id}…]}`) — **v2.12.6** | `draft:<draft_id>` | migration 088 (M3, L.C1.6) |
