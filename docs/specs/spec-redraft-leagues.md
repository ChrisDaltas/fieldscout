...
- Auction: `auction_budget ≥ roster_size × reserve`, where `reserve` = **1** with `auction_zero_dollar_nominations` OFF and **0** with it ON (every team can fill a legal roster). *(v2.13 — this bullet used to read `× auction_min_bid`; the field is gone and the reserve is now derived from the toggle. With the toggle ON the bullet is satisfied by every legal budget, which is the point: at $0 nominations nothing can price a team out of a roster spot.)* *(v2.13.2 — measured while building AP.1, and recorded so a future session does not read a live invariant as dead code: with the reserve capped at $1, the bullet is now satisfied by every SCHEMA-VALID settings object in BOTH columns, because `auction_budget` is ≥ 50 and `roster_size` tops out at 20 starters + 20 bench + 6 IR = 46. It stays enforced — never-weaken — as defence in depth at the API surface and as the thing the engine's own start gate mirrors on the D91 draftable-slot count, which is a DIFFERENT and smaller number and can still refuse. Exercised directly, one dollar either side, in `validate-league-settings.test.ts`; the engine-side pair is pgTAP 040 §D.)*
