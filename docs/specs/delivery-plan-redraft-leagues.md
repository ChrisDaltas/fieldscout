...
| **M3 — Auction engine** *(Phase C)* *(2026 test-cohort focus area, Chris 2026-08-03 — auction is a headline feedback goal; ships when its gate passes, no calendar deadline)* | L.C1 + v2.0 endgame rules & solvency invariant (§8.6.7–8); mock auctions (spec §8.8 CPU bidders) | Phase C gate + property test: *no reachable sequence of bids/undos/commish budget edits violates solvency — including bot-driven mock auctions* + bid-storm E2E |
...
| **M8 — Platform Admin Console** *(new — ruled by Chris 2026-09-09)* | A first-party operator surface for the PLATFORM owner, distinct from M6's per-league commissioner console. **The principle Chris stated, and the reason this is not more automation:** *"we probably won't predict the exact exception and will have to handle it differently — what we might actually need are some knobs and tools."* So the AUTOMATION owns the predictable path (the §11.2 week-close rule: release when every game is final, floored at Tuesday 00:00 PT, ceilinged at Wednesday 00:00 PT) and the OPERATOR owns the unpredictable explanation. Contents: platform-wide and per-league notifications composed and sent by hand (the `notifications` table and the rail's notifications panel already exist — 001, `rail/notifications-panel.tsx`); pause / resume / advance the scheduled jobs, waivers especially; force a week close or a finalize; a read-only view of a league's live state to decide what to do. **Depends on M5** (there are no waivers to pause before it). **Wanted BEFORE M7's GA**, not after — a postponed game during a real season is exactly when an operator needs these, and the beta ladder's first real scored weekends are inside M7. Placement in the ladder is Chris's call. | Every operator action writes an audit row and NONE can produce a state the §5 invariants forbid — or, where one legitimately must (a forced finalize over an unplayed game), the exception is explicit, named, and carries its own trail, so the synthetic gate's “zero final-cell rewrites” and “zero unhandled worker errors” arms stay honest · a rehearsed drill: a postponed game is handled end to end by hand, with the notification a real user would receive |
...
**2026 private alpha = a TEST cohort, not real leagues (ruled by Chris, 2026-08-03 — precedes the M7 ladder):** a small invite-only cohort of friends runs **test leagues** on the system through the 2026 season — free, feature-flag gated, explicitly not anyone's real league. 2026 is the **feedback year**: the alpha exists to gather unlimited feedback (and real recording-season validation), not revenue. Consequences:
- **No draft-season calendar pressure.** Test leagues can draft whenever a milestone is genuinely ready — there is no draft-night SLA, no week-1 scoring deadline, and the §5.1 in-season S0/S1 discipline applies from the first *real* league (2027), not from test cohort activity. Milestones keep their canonical order (M2 → M3 → M4 → M5) and their quality gates; feedback quality beats calendar dates.
- **Alpha focus areas: AUCTION and CUSTOM SCORING.** These are what Chris most wants nailed through 2026 feedback. M3 is *not* deferred; once it lands, auction test drafts become a primary feedback channel.
- **Custom scoring is un-punted for the alpha.** Spec v2.7 cut the custom scoring editor from v1 (6 parity templates only) to collapse the QA matrix; Chris's ruling partially reverses that: the test cohort gets custom scoring to iterate on. **Requires a spec changelog entry (Architect) before any builder implements it** — scope, QA matrix impact, and where it lands in the milestone map are the Architect's cut; §7.3.3 snapshot semantics (scoring frozen at draft start) remain LAW unless the spec says otherwise.
- The alpha does not change the GA target (July 2027); it feeds it — the M7 beta ladder (with real leagues) is unchanged.
...
- **v1.4 (2026-08-03):** 2026 private alpha added ahead of the M7 beta ladder (Chris's ruling, refined twice same-day before merge — snake-only draft → auction-priority draft → final: **test cohort**). Final shape: invite-only friends run **test leagues** (not real leagues) through the 2026 season, free — 2026 is the feedback year; no draft-season calendar deadlines, milestones keep canonical order (M2 → M3 → M4 → M5) and quality gates. **Alpha focus areas: auction + custom scoring.** Custom scoring editor un-punted for the alpha — needs a spec changelog entry (Architect) before build (partial reversal of v2.7's templates-only cut). GA target (July 2027) unchanged. Companion product ruling recorded the same day: the public go-live scope is Lists + Stats + AI lists only, free-only (Pro tier under rethink — CLAUDE.md business rules 2/3/6 suspended); leagues surface is alpha-gated.
