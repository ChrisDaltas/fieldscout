...
**Companion to:** `docs/specs/spec-redraft-leagues.md` (v2.6) — that spec defines *what*; this doc defines *how we build, test, fix, ship, and operate it* using Claude Code as the engineering team.
...
| **M1 — League foundation** *(Phase A + v2.0 settings)* | Tasks L.A1–L.A2 with the v2.0 settings catalog (schedule_mode/median/second, locks, correction window), scoring **snapshot** plumbing, `league_weeks`; identity contract + seat-targeted invites + `team_managers` stints written from day one (spec §7.2/§7.2.1/§12.22–23); seed the **8 scoring templates** + template-picker UX (spec §7.3.3/App B — no custom editor in v1, which collapses the scoring QA matrix); DL preset + Hot Swap naming in the roster builder | Spec §18 Phase A gate + snapshot present from draft start + settings round-trip tested for every §7.3 field + **template parity tests** (each platform template's calculator output matches hand-computed scores for 5 canonical player-weeks) + **Alpha/Ultra backtest harness runs** against recorded fixtures (`sim backtest`), report reviewed (OQ 17) |
