...
| **M3 — Auction engine** *(Phase C)* *(deferred behind M4/M5 per the 2026 private-alpha ruling, Chris 2026-08-03 — the alpha drafts snake-only; auction returns for the 2027 season)* | L.C1 + v2.0 endgame rules & solvency invariant (§8.6.7–8); mock auctions (spec §8.8 CPU bidders) | Phase C gate + property test: *no reachable sequence of bids/undos/commish budget edits violates solvency — including bot-driven mock auctions* + bid-storm E2E |
...
**2026 private alpha (ruled by Chris, 2026-08-03 — precedes the M7 ladder):** a small invite-only cohort of friends' leagues runs live on the system through the 2026 NFL season, snake-only, explicitly framed as testers. 2026 is the **feedback year**: the alpha exists to gather unlimited feedback (and real recording-season validation), not revenue — everything is free. Calendar consequences: **M2 must be draft-ready by early September 2026**; **M3 (auction) is deferred behind M4/M5** so in-season core (lineups/locks/scoring/standings) lands by NFL week 1 and waivers/trades shortly after — auction returns for the 2027 season. The alpha does not change the GA target (July 2027); it feeds it. Alpha leagues are feature-flag gated per §6; in-season S0/S1 discipline (§5.1) applies from the alpha's first scored week.
...
- **v1.4 (2026-08-03):** 2026 private alpha added ahead of the M7 beta ladder (Chris's ruling): invite-only friends' leagues run live through the 2026 season, snake-only, free — 2026 is the feedback year. Build order re-sequenced: M2 → M4 → M5 → M3 (auction deferred to the 2027 season); M2 deadline = draft-ready by early September 2026. GA target (July 2027) unchanged. Companion product ruling recorded the same day: the public go-live scope is Lists + Stats + AI lists only, free-only (Pro tier under rethink — CLAUDE.md business rules 2/3/6 suspended); leagues surface is alpha-gated.
