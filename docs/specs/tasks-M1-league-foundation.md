...
- **D36 — Username handling pending Q4.** M1 does NOT migrate `profiles.username` (retro-validity risk: existing 21–30-char handles). Invite-by-username matches case-insensitively via `lower(username)` (backed by a new `lower(username)` unique index — safe: all app writes are lowercased; migration verifies no case-duplicates first, stop condition if any). **The index ships in migration 043 (task L.A1.4).** citext conversion + 3–20 tightening + the changeable-vs-permanent contradiction are Chris's call (Q4).
...
-- SECURITY DEFINER SET search_path=''; in-body: auth + is_pro re-check + p_scoring_system_id
...
> 2. **D36's interim username index:** pre-check `SELECT lower(username) FROM public.profiles GROUP BY 1 HAVING count(*) > 1` returns zero rows (RAISE EXCEPTION and stop the migration if any — D36 stop condition), then `CREATE UNIQUE INDEX profiles_username_lower_key ON profiles (lower(username));`.
> 3. pgTAP 008: commish-only SELECT (manager/non-member/anon see zero rows against seeded fixtures); all client writes denied per role; token uniqueness; expiry default pinned (boundary: `expires_at` exactly 14 days from a pinned insert timestamp); D46 columns present; username index exists + case-variant duplicate insert fails 23505.
> 4. Typegen + alias (`LeagueInvite`).
>
...
>
> 1. `create_league` RPC per the §5 sketch (single txn: league + member (faab_balance seeded from faab_budget, §12.2) + team + stint + invite_code; max_teams sync; is_pro re-checked in-body — the Route Handler's `requireProUser()` is UX, the in-RPC check is security per §12.0; **in-body: `p_scoring_system_id` must reference a `scoring_systems` row with `is_template = TRUE AND owner_id IS NULL`** — v1 templates-only (§7.3.3), friendly per-field error otherwise).
...
> 4. Integration tests (local stack): create at every team_count (8..16 — the Phase A gate sizes); creator seated as commissioner with team + open stint (stint asserted directly; `faab_balance = faab_budget` asserted); non-Pro 402 `PRO_REQUIRED`; **negative: a personal (owner-scoped, non-template) scoring_systems id → rejected, no league row written**; invalid settings 400 with the per-field message; idempotency-safe double-submit (natural key or action_id — client retries must not create two leagues).
>
...
> 3. Email seam (D37): `EmailSender` interface + dev implementation; invite email = league name, team label, claim link `/join/[token]`; send recorded on the invite row (`created_at` = initial send, `last_sent_at` on re-send — D46). In-app notification for username invites (existing notifications infra, new `league_invite` type).
...
|---|---|---|---|
| **C1** | **CLAUDE.md Key Business Rule #5** says "Leagues are Pro only. Gate league creation **and joining**" — the spec says joining is **free** (stated 3×: §3.1.6, §7.2, §19.1) | CLAUDE.md rule 5; spec lines 60/169/1637 | Spec wins (CLAUDE.md Active Builds rule). L.A1.14 ships join un-gated. **Chris: edit rule 5** to "creation Pro-gated; joining free" (Q6) |
