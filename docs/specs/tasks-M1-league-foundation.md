...
>
> **Q4 ruling (Chris, 2026-07-20) folds the username contract into this task's schema work:** `profiles.username` CHECK 5–20 + the `lower(username)` unique index (D36's 043 slot is superseded — the index moves here). **⛔ HALTED 2026-07-20 (Builder session, stop condition):** recording the ruling surfaced two codebase conflicts the §9 report missed — the account-settings page ships a live username-*change* flow, and AI persona accounts hold hyphenated `*-ai` handles in `profiles.username` (prod rows) that violate the charset the CHECK would inherit. **PROGRESS Q7** carries the questions + recommendation; this task resumes when Q7 is ruled. The leagues-columns half is unaffected by Q7 but ships with the Q4 half in one migration per the ruling.
>
...
- **Chris (PROGRESS §3):** ~~Q4–Q6~~ **ruled 2026-07-20** (recorded in spec v2.8 + this doc). **Q7 OPEN** — Q4-application conflicts (persona `*-ai` charset exemption + settings-page rename flow + the "creation vs selection" permanence boundary); blocks L.A1.1 and the schema lane behind it. Still his: C2 (stale CLAUDE.md redesign bullet) and C13 (spec-leagues-live-mode superseded banner).
