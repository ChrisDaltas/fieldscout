...
| 061 | `061_update_league_settings.sql` *(added 2026-07-25)* | `update_league_settings` RPC (§7.3-header status gate, Q10 backstops, templates-only, snapshot re-freeze, `max_teams` sync, §12.2 re-seed); **F28 shrink floor amended in 2026-07-25 by L.A1.14** | L.A1.13 (+ L.A1.14 amendment) |
| 062 | `062_league_invites_rpcs.sql` *(added 2026-07-25)* | Invite RPCs: `create_league_invite` / `revoke_league_invite` / `rotate_invite_code` / `set_league_invite_slug` / `get_join_preview` (anon carve-out) / `claim_league_invite` / `join_league_by_code` + 2 locked internal helpers (seating, guarded notify); **amended in place 2026-07-26 by the batch-14 remediation** — `revoke_league_invite` gained `p_league_id` (R86; signature `(uuid,uuid)`, typegen re-run) and the three commish RPCs' deleted-league branches raise P0002 instead of P0001 (R87, the 059/061 convention) ; **amended again 2026-07-26 by L.A1.15** — `seat_league_member_internal` gained `p_team_id` and a placeholder **FILL** (a §7.2:170-shaped placeholder seat was otherwise UNCLAIMABLE: `UNIQUE(league_id, team_id)` raised 23505, which claim swallowed as `seat_filled` and used to expire the invite), and `claim_league_invite`'s seat-targeted branch now routes through that one helper (D74(1)(2)) | L.A1.14 (+ batch-14 and L.A1.15 amendments) |
| 063 | `063_league_member_management.sql` *(added 2026-07-26)* | Member management: `add_placeholder_seat` / `set_member_role` (incl. the atomic commissioner transfer) / `assign_manager` / `remove_manager(mode)` / `leave_league` + 1 locked internal notifier; R43-class integrity (`league_members.role` CHECK, `one_commissioner_per_league` partial unique index, `team_managers.end_reason`/`role` CHECKs); **drops 054’s two placeholder-seat client policies** — `league_members` is SELECT-only on the client (spec erratum v2.8.8) | L.A1.15 |
