...
| 062 | `062_league_invites_rpcs.sql` *(added 2026-07-25)* | Invite RPCs: `create_league_invite` / `revoke_league_invite` / `rotate_invite_code` / `set_league_invite_slug` / `get_join_preview` (anon carve-out) / `claim_league_invite` / `join_league_by_code` + 2 locked internal helpers (seating, guarded notify) | L.A1.14 |
