...
>
> Read ALSO the 066 banner (L.B1.2) — the **R125 routing lands here** (M2 batch 2): `draft_make_pick`'s E2 replay arm does NOT filter `is_undone`, so replaying an action_id whose pick was later undone returns a success-shaped `{draft, pick(is_undone=true)}` no-op. This task decides replay-vs-undone semantics when `draft_undo` lands (and pins the decision).
>
> Read ALSO the 068 banner (L.B1.3) — the **auto-start↔reset seam lands here** (D107): `draft_reset` returns draft + league to `scheduled`, and under a PAST `settings.draft.draft_scheduled_at` the D94 auto-start arm will RE-START the reset draft on the next 5s tick. This task owns reset semantics and must close the loop (clear/require re-scheduling of the instant, or an equivalent recorded gate) and pin it.
>
...
> Read ALSO PROGRESS **M2 batch 9 (R155/R156)**: add one OUTSIDER (non-member) client to the route-auth sweep — `draft-api-db.test.ts` pins none of the no-leak arms (outsider PATCH 404 / create+start 403, reviewer live-verified) (R155); and in `draft-service.ts`, the randomize path's empty-active-teams 404 misuses `NO_ACTIVE_DRAFT_MESSAGE` for a corruption-only state — make it an invariant 500 or drop the branch with a comment when this file is next open (R156).
>
...
> Read ALSO PROGRESS **D110(1)** (L.B1.6, spec v2.8.16): on a MOCK draft, `draft_pause`/`draft_resume` are LAUNCHER-only (commissioners refused — D103(2)) and every other §8.7 control refuses mocks outright — so the commish control routes need no mock affordances, and the mock surface (launch/list/delete per §15.2 + pause/resume for the launcher's own mock, riding the SAME two RPCs) is authorized by `config.mock.launched_by`, never by role. The launcher's resume path is how an E59 auto-paused mock comes back (L.B3.5's resumable card is the consumer). Read ALSO **D110(11)** (batch 7, R149): the launch route (`POST …/mock-drafts`) mints ONE `p_action_id` UUID per submit — the D68(1) stamping pattern — and passes it to `create_mock_draft`; the RPC's E2 replay arm returns the original mock (`created: false`) on a retried submit, so the route treats created=false as the same 2xx (never a duplicate-launch error). This route owns launch dedupe end-to-end; NULL is the RPC's legacy no-dedupe path and production routes never send it.
> Read ALSO PROGRESS §6 row **F51** (R157, M2 batch 10) — **this task discharges it**: amend 065 IN PLACE (unreleased, F12) to exclude `is_mock` drafts from `draft_queues`' owner arm (BOTH policies — "Own queue read" + "Own queue write"), leaving the D103(3) launcher arm as the ONLY mock admit. The open owner arm is a live D103(2) breach: the human seat's real owner can inject queue rows over raw PostgREST that steer the launcher's next human-seat autopick (068's resolve is writer-blind, `ORDER BY q.rank`) and read the launcher's practice prep. Pin per-role in pgTAP 019 from both sides (seat-owner INSERT/SELECT refused on a mock; launcher path intact; real-draft own-queue arm untouched); the L.B2.2 route seam (`resolveQueueTeam`) stays. Flip F51 in the same PR.
>
...
> Read ALSO PROGRESS **D108(15)** (R137, M2 batch 5; findability line per R142, batch-5 addendum): `league_chat.user_id` is `ON DELETE SET NULL` for ALL rows, so **ordinary (`is_system = FALSE`) chat messages can be AUTHORLESS** — a deleted account's rows keep their text with `user_id` NULL, a row shape the R137 fix created. This task's chat UI must render the authorless-ordinary shape (the "former member" author fallback) as well as the tick's `user_id`-NULL system posts (which render actorless by their `is_system` shape and name any actor in the text).
>
...
>
> Read ALSO PROGRESS §6 row **F49** (L.B1.3/D107(8)) — the **fixture-instant sweep lands here** (unless a September 2026 ops session ran first): 068's `draft-tick` cron is LIVE on any running stack and auto-starts COMMITTED `scheduled` leagues whose stored `draft_scheduled_at` has passed (the D94 contract working as designed). The wire/gate suites pin literal instants — `draft-core-db.test.ts` 2026-09-01 · `m1-phase-a-journey.test.ts` 2026-09-13 · `settings-round-trip-db.test.ts` 2026-09-03 · `round-trip-fixture.ts` 2026-08-30 — so from those dates the background cron can flip a suite's committed league `scheduled`→`drafting` MID-RUN. Sweep the instants to far-future values (2027+) or equivalent discipline BEFORE composing `test:gate:m1` into this gate; flip F49 in the same PR.
>
...
- **M3 inherits:** `draft_bids` + `draft_nominate`/`draft_place_bid` + the auction `draft_tick` arm (anti-snipe, endgame E25–E27, solvency E28/E62); the auction-draft-room re-skin (+ its standing lint warning at auction-draft-room.tsx:105); CPU auction bidders (value-based, same solvency validator); E29's auction half (cascade during live bids); `nomination_order` machinery. Also: the presence-only `realtime.messages` INSERT posture (D109(5)/070 banner) is deliberate — Client Broadcast is closed so a member cannot forge a server-shaped room event; M3's auction realtime decides whether to open it (a deliberate policy extension) for bid-pulse/typing UX.
