...
> 4. pgTAP **036**: E28 all three arms + positive control (legal edit lands, `budget_adjustments` map updated, solvency holds); reverse-won-bid golden (pick undone, pool restored, rotation re-includes, derivation reflects instantly); E29 cascade golden (nomination with 3 live bids + undo to seq N → nomination voided, picks > N undone, on_clock = seq-N nominator); priced-reassign refusal (receiving team over max-bid) + positive control; per-control non-commish 42501 sweep + mock-refusal sweep (D138); system post pinned per control.
>
...
> 3. **`auction-draft-room.tsx` DELETED; `mock-draft.ts` DELETED** (its banner's own instruction — the auction fixture types are replaced by live shapes); the standing lint warning retires — `npm run lint` output goes clean-or-new-baseline, recorded.
...
| **C34** | Bid-clock semantics ambiguous: §8.6.3 "each bid resets the bid clock per anti-snipe" vs §7.3.8/E6/App C "resets clock **to** anti_snipe_seconds" (a reset-to-full reading contradicts E6 as printed) | spec §8.6.3 vs §7.3.8:376–377, E6, App C L.C1 | **D128**: fixed window + anti-snipe floor (every printed number is consistent with it). **NEEDS CHRIS BEFORE BUILD** — auction pacing is product feel; Sleeper-style reset-per-bid would be a spec change with E6 rewritten |
...
- **Chris:** **C34** (bid-clock pacing — before build) · **C37** (custom-scoring changelog entry as a separate parallel Architect track — before/alongside approval) · F47 (CI) · C33's erratum read (with the PR) · the C19/C2/C13 leftovers from earlier milestones remain his.
