...
> **CORRECTION, 2026-08-24 (MP.1 / PROGRESS D234) — two of the three figures above are wrong, measured against the deployed schema at `093`.**
> **(a) In-body auth sites: EIGHT executable on a mock's path, not eleven.** The `length/replace` count returns 13, but **four of those are COMMENT lines**, not calls (`draft_make_pick`, `draft_nominate`, `draft_touch`, `set_team_autodraft` each carry an `-- … is_league_member(NULL) is FALSE` note). Filtering `trim(line) !~ '^--'` gives **9 executable sites across 9 functions, one each**; `set_team_autodraft` is a league verb → **eight on a mock's path**, and `create_mock_draft` is superseded by MP.3's new launch RPC → **seven bodies amended**. **This is D228(3)'s error species one level down** — D228 counted superseded bodies, this counted comments. **Count what EXECUTES, not what MATCHES.**
> **(b) RLS: FOUR required SELECT arms, not five.** `league_chat`'s INSERT arm is optional because **every engine write bypasses RLS** — the 58 functions are owned by `postgres`, `pg_roles.rolbypassrls` is true for it, and `relforcerowsecurity` is false on all seven tables. It is needed only if a human types into a mock room, and D226(2) says there is nobody to type to.
> **(c) The four `NOT NULL` drops are CONFIRMED exactly** — and `league_chat`'s is **load-bearing**: `draft_pause_internal` and `draft_resume` write chat with `v_draft.league_id` **unguarded by `is_mock`**, so a standalone mock raises **23502 on pause and on resume**. **(d) Two costs this section never counted at all:** the **five** FK children of `drafts` (incl. `draft_dnd_marks`), and the **two `realtime.messages` `draft:%` policies** that gate on `is_league_member` — without an arm the launcher cannot subscribe to their own room. **(e) A THIRD BREAKS-ON-NULL CLASS, FOUND AT REVIEW (R484) AND LARGER THAN BOTH — the `leagues`-EXISTENCE GUARDS.** `EXISTS (SELECT 1 FROM public.leagues l WHERE l.id = … AND l.deleted_at IS NULL)` appears on **14 non-comment lines of `draft_tick`** (plus `draft_start_internal` 3; `create_mock_draft` / `draft_autopick_resolve` / `draft_create_internal` / `draft_reset` / `draft_set_order` 2 each; `draft_complete_internal` / `draft_end` / `draft_make_pick` / `draft_mock_cpu_need` / `draft_nominate` / `draft_place_bid` 1 each). **SIX of `draft_tick`'s sit INSIDE its `AND d.is_mock` scans** — body lines **268/291**, **531/559**, **680/722** — so with no league **the tick never claims a standalone mock: no CPU picks, no nominations, no bids, and NO ERROR** (probed live: `tick_leagues_guard` `t` / `t` / **`f`** over REAL / MOCK-in-league / MOCK-standalone). **And `draft_make_pick`, `draft_nominate` and `draft_place_bid` each carry a SECOND, uncounted `IF NOT EXISTS (… leagues …) RAISE … P0002` immediately after the `is_league_member` site counted in (a)** (`PERFORM … FOR UPDATE; IF NOT FOUND` variant in `draft_end` / `draft_reset`) — probed `raises_P0002` `f` / `f` / **`t`**, so the human's own pick would 404 on their own practice. **So "seven bodies amended" is seven bodies PLUS this class, and MP.3 is materially larger than (a) implies. It is MP.3's, not MP.2's** — MP.2 owns the two *settings readers*; these are *existence and candidate-set predicates* in different bodies. **F109** carries (c), (d), (e), the chat pane's league-keyed READ (`use-draft-chat.ts:44–50` — `enabled` on `leagueId` and `.eq('league_id', …)`, so a standalone launcher cannot read the engine's own SYSTEM posts; R490) and the §12 spec erratum, all to MP.3. **The 6,252-line figure is fine** — it is 6,310 lines counted as newlines (6,310 − 58).
...
*(**ANSWERED 2026-08-24 — PROGRESS D234: SAME TABLES, `league_id` nullable, the ownership arm scoped by `league_id IS NULL`; mock-owned tables REFUSED. Item 4's figures are corrected by the §1.3 correction note above — eight executable in-body sites, four required SELECT arms. D233(5) is answered PROVABLY IDENTICAL, with the deliberate break shown RED and reverted. `MP.2` is now a HARD prerequisite of MP.3.**)*
...
3. **Re-point, do not fork:** `mockLauncherHref` (`mock-launcher-entry.ts:13–19`) — **AMENDED 2026-08-24 at MP.6 review (R522): the `mockLauncherHref` re-point belongs to MP.6c, NOT here, and MP.6 deliberately left it league-side.** It resolves *"Practice this draft"* on a LEAGUE's page (`?practice=1`), which is the **deferred** league-attached feature (§6) and still mounts league-side after MP.6; re-pointing it at `/app/mocks` would have sent a league mock's launcher to the standalone home while the room it opens had not moved. MP.6 added the **standalone arm beside it** instead (`mockRoomHref`, and at R521 `leagueMockRoomHref`), which is what *"do not fork"* asks for; the seam re-point lands with the room in **MP.6c**. **A Builder must not "fix" this back.** Also: `room-entry.test.ts`, `room-exits.test.ts`, and `route-groups.test.ts:223–232`, which gains the mock room's own gate assertion beside the two leagues ones, **and declares `/app/mocks/[mockId]` in `APP_URLS_ADDED_SINCE_GOLDEN`** with the note that file's own comment already forecasts. **A second room component is the LV.7 failure pattern.**
...
> **BUILT 2026-08-25** (`feat/MP-MP.6c-league-optional-room`; PROGRESS **D247**; spec erratum **v2.16.4**). **ONE room, TWO mounts, and the room cannot tell which it is:** the non-draft context is a `RoomScope` object with two fill sites (`room-scope.ts`), and the rule is pinned by ABSENCE — `useLeague(` appears once in `draft-room.tsx`, above both mounts, and the room reads no `drafts.config` of its own, so the deferred league-attached mock is a THIRD fill rather than a rewrite. **Item 3's guard needed the scope, not a mock arm** (`draft.league_id !== scope.leagueId` is stricter on both arms). **Item 6's exits moved by measurement**: 7 `Back to league` literals → 1, and `room-exits.test.ts` now pins scoped exits plus an enumeration of the file's three remaining `/app/leagues` literals. **`ROOM_URL` gained its `/app/mocks/${…}` arm (R520)**, with **F123** filed for `MockRow`'s entry-split disposition. **F120 took a migration (097) and the PR says why a client transform was refused**; **F122 was answered with the server's own ownership predicate** and folded to the spec. **Item 9 was performed in full, twice to completion** (a standalone snake and a standalone auction, `leagues` OFF), and a league room was re-driven with `leagues` ON with nothing moved. **Item 10 was not engaged: no new region, no resized zone, no new breakpoint** — every state reuses the shipped `Card`/`DraftRoomEmpty`/`DraftRoomProblem`/`DraftRoomSkeleton` treatments, so DR2 was not the addressee.

> **RESOLVED 2026-08-24 — `F121` IS FIXED (migration **096**, `fix/MP-F121-standalone-uncontestable`; D246), so ITEM 9's AUCTION ACCEPTANCE IS NOW PERFORMABLE IN FULL.** A standalone practice auction has bidding: `draft_nomination_uncontestable` gained the `league_id IS NULL` arm over the mock's own `drafts.draft_order` (095 banner item 4's idiom, NOT the ruled-wrong `IS NOT DISTINCT FROM` one-liner), pgTAP **044** pins it with two mocks in the fixture, and `standalone-actions-db.test.ts`'s bid test is now a real **200 raise over a bot's standing high bid** with the D128 anti-snipe floor measured on it. **The MP.6c Builder should expect to see bots raising in the room** — and the one thing to NOT write up from intent is the tick's `auction_cpu_raised` counter, which is a fold counter in practice (091/AP.3 answers every bid inside the bid transaction; the sweep is the safety net). The original finding is kept below for the reasoning it carries.
>
> **ADDED 2026-08-24 at MP.6b's landing (D245(7)), because item 9's acceptance cannot be performed without it: `F121` — on today's engine a STANDALONE AUCTION HAS NO BIDDING.** `draft_nomination_uncontestable` filters `t.league_id = v_league_id`, so with a NULL league every nomination is declared uncontestable and awarded instantly at its opening bid (measured on a DB holding **two** standalone mocks: the predicate answers `t`, and the **fix is 095's banner-item-4 idiom — a `league_id IS NULL` arm over the mock's OWN `drafts.draft_order`, never an `IS NOT DISTINCT FROM` widening**, which that banner already rules wrong for this unanchored `FROM public.teams` scan and which `draft_team_budget` raises on). Item 9 says *launch a standalone auction … **watch the bots act***; there is nothing to watch until F121 is fixed, and a Builder who reaches this task must **not** write that state up from intent (§4 rule 9). ~~**Take F121 first, or drive the auction half only as far as the engine honestly goes and say so.**~~ *(Done — F121 landed first, as recommended.)* **Also inherited: `F122`** — item 2 lists the room's non-draft context sources (seats, roster, scoring) and does **not** mention LISTS; §8.9's *"My Lists"* panel is a league surface, and the server rule for a standalone mock is now OWNERSHIP (spec **v2.16.3**). Decide what the panel shows, and do not offer what the verb would refuse (D110(1)).
...
6. **The exits, and they are the R340 invariant restated for a room with no league.** **ADDED 2026-08-24 at MP.6 review (R520) — do this in the same task: `room-entry.test.ts`'s `ROOM_URL` sweep (`:70`) matches ONLY `/app/leagues/${…}/draft` templates, so the moment the room mounts at `/app/mocks/[mockId]` the DR.6 entry-split invariant silently stops covering it.** Give `ROOM_URL` its `/app/mocks/${…}` arm and disposition every new hit, exactly as the file's own docblock requires. *(Harmless until then — MP.6 opened the URL but the room still mounts league-side, and MP.6 dispositioned the one entry site it did add. This is the line that keeps that true.)*  `draft-room.tsx` carries **7** `Back to league` links; `room-exits.test.ts` pins **≥4** surviving `PageHeader` deletion, plus the takeover and skeleton states by name. **On the standalone arm every one of them goes to `/app/mocks`, never to a league** — and `room-exits.test.ts`'s five-link inventory pin (MP.5/F119) **moves by measurement here**, exactly as its own comment says it should: *"If a later change makes either surface league-optional, this count moves and the decision above gets re-taken."*
