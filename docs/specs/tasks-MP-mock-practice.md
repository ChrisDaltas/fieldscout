...
- **The seat refusal is `092:1233–1241`**, the D103(1) block:
...
- **Three other refusals sit in the same function and are all CORRECT — do not touch them:** the pre-draft-league gate (`092:1208–1214`), the §22.5 caps (3 active mocks `092:1246–1255`, 5 creations/hour `092:1257–1266`), and the §8.6.8 auction-solvency backstop (`092:1400–1411`).
...
- **Two cards, both grouped by team.** *"Final board"* — `DraftBoardGrid` for snake/linear, `AuctionFinalBoard` for auction (`draft-recap.tsx:264–283`) — and *"Your roster vs the CPUs"* / *"Rosters"* (`:284–312`), a grid of `RecapRosterCard`s in `recapTeamOrder` (`draft-recap-ops.ts:94–118`).
- **Prices and per-team totals already exist.** `recapTeamSpend` (`draft-recap-ops.ts:130–152`) returns `{ total, biggest }` per team; `RecapRosterCard` calls it at `draft-recap.tsx:334`.
...
- **`MORE_ITEMS` is `src/components/layout/sidebar.tsx:57–74`**: Community (`featureFlags.community`) and My stats. It is consumed twice in the same file — `:141` (`moreActive`) and `:250` (the render) — and **`grep -n "MORE_ITEMS" -r src` returns exactly those three lines**, so nothing else imports it.
...
- **`featureFlags.leagues` currently gates everything mock-adjacent** — the Home league cards (`(shell)/page.tsx:27`), the sidebar league list, `HomeQuickActions`' first two chips, and `more-sheet.tsx:63–64`'s Leagues row. **Its docblock is stale** — *"Leagues, league teams, and the live draft — mock-data UI, no backend yet"* (`:20`) — which has not been true since M1. Filed as **F103**, not drive-by fixed.

### 1.8 THE ROOM'S ROUTE IS BEHIND THE LEAGUES GATE — the fact that decides whether this lane's headline property is achievable

**This section was added after review (R470) and it is the most consequential measurement in §1**, because without it the lane folded LAW (E79) that no task could satisfy.

- **Every `/app/leagues` URL is hard-redirected when the leagues flag is off, and the live draft room is one of them.** `src/app/app/(room)/leagues/layout.tsx:18` is `if (!featureFlags.leagues) redirect('/app')`. Its docblock (`:5–12`) says so in as many words: *"every /app/leagues route (index, workspace, live draft) is unreachable — even by direct URL"*, and it exists precisely because DR.1 moved the live draft out of `(shell)` and the original gate would otherwise have missed it.
- **The mock room and the launcher both live under that gate.** `mock-launcher-entry.ts:18` builds `/app/leagues/${leagueId}/draft?practice=1`, and `find 'src/app/app/(room)' -type f` returns **three** files: `layout.tsx`, `leagues/layout.tsx` (the gate) and `leagues/[leagueId]/draft/page.tsx` (the room). **There is no room route outside `/app/leagues`.**
- **Both gates are pinned.** `src/lib/route-groups.test.ts:223–232` loops over `(shell)/leagues/layout.tsx` and `(room)/leagues/layout.tsx` and asserts each contains `featureFlags.leagues` and `redirect('/app')`.
- **So `featureFlags.mockDrafts` alone cannot make the room reachable**, and D231(3)'s pin — Home entry, room, report and the More… tab all working with `leagues` **off** — is unsatisfiable at the route layer until a mock room exists outside `/app/leagues`. **That is MP.10, and it is a prerequisite of MP.4, not a polish task.**
- **A second consequence, which is D228's problem rather than the gate's.** Under the practice container, a standalone mock's URL today would be `/app/leagues/<container-uuid>/draft` — **putting the id of the thing D228(1) calls hidden into the address bar of every standalone practice.** MP.10's route keys on the **mock's own id**, which removes the container from the URL entirely.
- **Measured: there is no conflict with PR #180.** `git diff main...pr180 --stat` returns **five files, all under `docs/`** — DR2's PR touches **no code at all**. DR2's unbuilt tasks work inside `draft-room.tsx`; MP.10 moves the 20-line route wrapper around it. **Moving a route is not moving a layout** — §4 rule 15 is not engaged, and MP.10 says so explicitly so a Builder does not stop on it.

### 1.9 Q23, AP.4 and B5 — the state this lane inherits
...
|---|---|---|---|---|
| R | 469 | 455 | 469 *(470/478 appear only inside D224's own correction-of-record and name no finding)* | *(none — Architect sessions file no findings. **The review round of this PR opened at R470**, so the next reviewer takes the number after its last.)* |
| D | 223 | 212 | **224** | **D225–D232** |
| F | 101 | 97 | 101 | **F102–F106** *(F106 added by the review round)* |
...
2. **Task ids are `MP.1`–`MP.10`.** No `L.C`, `AP.`, `DR2.`, `SE.` or `MS.` id is reused, retired or renumbered. `MS.*` keeps every task it has (§7).
3. **Numbers taken: D225–D232, F102–F106, Q24, E78–E80, spec v2.16.** *(F106 was added by the review round — R474; the R-numbers themselves belong to the reviewer, who takes R470+.)* (§1.10's sweep, over `main` and **both** open PRs).
...
5. **BUT "by construction" IS ABOUT LEAGUE ISOLATION ONLY, AND SAYING IT UNQUALIFIED WOULD BE FALSE (R473 — live-probed in a rolled-back transaction).** `teams` carries two policies that a `league_id IS NULL` row lands squarely inside: **"Teams are viewable by everyone"** (`001:844`, SELECT `true`) and **"Users can manage own standalone teams"** (`053:69–71`, `FOR ALL USING (auth.uid() = owner_id AND league_id IS NULL)`). Probed as `authenticated`, the launcher got **`UPDATE 1`** and **`DELETE 1`** on their own CPU seat. And `draft_picks.team_id`'s FK is plain `REFERENCES teams(id)` (`065:160`) — **NO ACTION** — so deleting a seat *before its first pick* is permitted and **wedges the mock at that seat's turn**. The same exposure covers a standalone mock's **human** seat. **Two honest dispositions, and MP.1 must take one explicitly rather than inherit the omission:** **(a)** accept it under the standing *"in drafts that's known as a 'personal problem'"* ruling — a launcher who deletes their own practice's CPU has broken their own throwaway, and the blast radius stops at their mock; or **(b)** have MP.1 narrow 053's standalone-teams policy to exclude rows named in a live mock's `cpu_seats`. **Recommendation: (b) for DELETE, (a) for UPDATE.** A rename is cosmetic and self-inflicted; a delete **wedges a draft with no in-product recovery**, and "renaming a CPU is your business, deleting one mid-draft is not" is a line the policy can express. **World-readability is accepted either way and is stated rather than hidden:** a CPU seat's name is visible to any authenticated reader who has its id, exactly as every other `teams` row is (§17), and it carries nothing but a label.
6. **The human's seat is unchanged for a league-attached mock** — `092:1268–1292` still resolves the launcher's real franchise by default and still allows any active seat. Only the *empty slots* become CPU seats.
7. **Naming is a product surface and gets one rule, not a system.** CPU seats are named `CPU 1 … CPU N` unless the league's own placeholder naming already applies; no avatars, no personas, no generated team names. Chris ruled no explanatory copy for MS (D221(1)) and the same restraint applies — this lane adds no flavour it was not asked for.
8. **Deletion order matters and is a Builder trap.** `draft_picks` cascades from `drafts`, and `teams` has **no** cascade from `drafts`; so `delete_mock_draft` and `mock_draft_expire` must delete the draft **first** and the `cpu_seats` teams **second**, in the same transaction, or the FK refuses.
...
3. **THE CONTAINER IS ONLY HIDDEN IF ITS ID IS NOT IN THE ADDRESS BAR, AND TODAY'S ROUTES WOULD PUT IT THERE.** The room is `/app/leagues/[leagueId]/draft`, so a standalone practice's URL would read `/app/leagues/<container-uuid>/draft` — publishing the id of the row this decision calls hidden, on every practice, to anyone glancing at the screen. **`MP.10` keys the mock room on the MOCK's own id** (`/app/mocks/[mockId]`), which removes the container from the URL entirely. That is not a side benefit of R470's fix — **it is a condition of this decision being honest**, and it is recorded here so a Builder who reads D228 without reading §1.8 still meets it.
4. **What the container costs, stated so it is not discovered later.** One additive `leagues` column (or one `settings` key) plus an exclusion predicate on `listMyLeagues` (`leagues-service.ts:437`) and on every future league-enumerating surface — filed as **F105**, because "every future surface" is exactly the shape of a forward obligation.
5. **§8.8's isolation rule is UNAFFECTED and its meaning is sharpened, not weakened.** §8.8 protects **the real league** a mock is attached to. A practice container is not a real league: nobody else is a member, it is never enumerated, it holds no rosters, no matchups and no invites, and it is destroyed with the practice it backs. **A standalone mock has no real league to isolate from, so its isolation obligation is vacuous — and MP.9 proves that vacuity rather than assuming it** (a whole-schema delta showing that a standalone mock's entire lifecycle touches nothing outside its own container and its own draft rows).
6. **THE CONTAINER'S SINGLE MEMBER ROW CARRIES `role = 'manager'`, NOT `'commissioner'` (R479 — decided here rather than left to a Builder).** `is_league_commish` is role-based — `SELECT ... WHERE m.league_id = $1 AND m.user_id = auth.uid() AND m.role IN ('commissioner','co_commissioner')` (`052:96–105`) — and the obvious precedent a Builder would copy, `create_league`, inserts `'commissioner'` (`077:344–345`). **Copy it and the container becomes a fully functional league whose only protection is that nothing enumerates it**, which is protection by obscurity in a codebase whose entire authorization model is in-body and role-based. `'manager'` makes `is_league_member` answer — which is all D228(1) actually needs for RLS — and makes **the whole commissioner family refuse the container by its own existing gate**, with no new predicate to remember. **The launcher loses nothing:** authority over a mock is keyed on `config.mock.launched_by`, never on a league role (D103(2)/MS's D217(3)), so every MS control the launcher is meant to hold still answers. **The residual, stated:** `leagues.owner_id` is still the user, and any surface keyed on ownership rather than role would still see the container — **F105's enumeration obligation covers ownership-keyed reads too, and MP.2's Builder must check `owner_id` call sites as well as `league_members` ones.**
7. **The container is per USER and reused, not per mock.** §22.5's 3-active/5-per-hour caps already bound the mocks; minting a `leagues` row per launch would put an unbounded write behind a button. One container, whose `settings->'draft'` is rewritten at each standalone launch, is the cheaper and more auditable shape.
8. **Recorded as a recommendation Chris can overturn at this PR's review** — the D220 shape. It is a build-mechanics call with a product-visible edge (a hidden `leagues` row is a thing that exists in the database and did not before), and that edge is his to accept or refuse.
...
3. **The pin is the assertion that keeps it true, and it is not optional.** With `leagues` **off** and `mockDrafts` **on**: the Home entry point renders and launches, **the room loads**, the More… tab renders, `/app/mocks` lists, and a report opens. MP.9 owns the end-to-end version; MP.4 and MP.6 each carry their own half. **A lane that states an independence property without a test for it has stated an intention.**
3a. **AND THE ROOM HALF IS REFUSED BY A SHIPPED, PINNED ROUTE GUARD UNTIL `MP.10` LANDS (R470).** `(room)/leagues/layout.tsx:18` hard-redirects every `/app/leagues` URL when the leagues flag is off — *"even by direct URL"*, by its own docblock — and the mock room is one of those URLs (§1.8). **`featureFlags.mockDrafts` cannot reach past it**, because the gate is a different flag on a different layout. **This was missed in the first cut of this document and is the reason MP.10 exists**; the failure mode is the one worth keeping: *a release-independence claim is a claim about ROUTES as well as about components, and the route layer had a guard the component layer could not see.*
...
10. **§8.8's ZERO-SIDE-EFFECTS RULE IS THE GATE ON EVERY TASK, AND IT IS ANSWERED BY MEASUREMENT.** No MP task may create, delete or re-route a mock without, in the same PR, an isolation proof taken around the action: the **whole `leagues` row** and the **whole real `drafts` row** of any attached league, plus a **whole-row composite** (not a count) over every league-scoped table the write set or trigger graph can reach. *"It only writes `teams` with a NULL `league_id`"* is a reading, not a measurement. **For a standalone mock the obligation is not skipped — it is proven vacuous** (D228(5)): a whole-schema delta over all 56 `public` tables showing the entire lifecycle touches nothing outside the container and the mock's own rows.
...
**Why ten tasks.** Three are server (**MP.1** the seat primitive, **MP.2** the standalone mock, **MP.9** the closing sweep); six are surfaces (**MP.3** the gate + the practice home, **MP.4** Home, **MP.5** the report, **MP.6** the nav, **MP.7** League Home, **MP.10** the room's route); one is the route split (**MP.8**). **MP.10 was added by review (R470) and is numbered last but sequenced early** — it gates MP.4, because without it the lane's headline property is refused by a shipped route guard (§1.8). **MP.1 is first and is not negotiable** — it is a live bug Chris hit in production use, it blocks nothing else conceptually, and it builds the primitive MP.2 needs.
...
5. **`delete_mock_draft` (`071:469–515`) and `mock_draft_expire` (`071:524–610`) both gain the cleanup**, in the correct order (D227(8)): delete the draft first (picks/bids/queues cascade), then `DELETE FROM public.teams WHERE id = ANY(<cpu_seats>) AND league_id IS NULL`. **The `league_id IS NULL` predicate is a safety belt, not decoration** — it makes a malformed `cpu_seats` array incapable of deleting a real franchise.
...
4. **The launch RPC — AND THE CHOICE IS NOT THE BUILDER'S (R478).** **It is `create_mock_draft` gaining a nullable `p_league_id` arm, not a `create_practice_mock` sibling.** The reason is the cross-lane seam: **merged MS.8 says its slot parameter *"rides `create_mock_draft`"*** (`tasks-MS-mock-sandbox.md:447`). Pick the sibling and a standalone mock **silently loses slot choice** — the one thing D223 exists to give a launcher — because MS.8's parameter would land on the other function. **Three tasks across two lanes `CREATE OR REPLACE` this one function (MP.1, MP.2, MS.8), so it is serialized like any migration lane and each is authored against the chain HEAD's file text** (D137/CLAUDE.md). One seat-synthesis implementation and **one** order resolver (`draft_resolve_order_internal`, whose HEAD is `066`; the mock's call site is `092:1304–1325`, seeded by the mock's own id). `SECURITY DEFINER`, `search_path=''`, in-body auth, REVOKE. §22.5's caps apply unchanged and are counted the same way.
...
6. **pgTAP (~043).** A user in **zero leagues** launches a standalone mock, drafts, completes, and deletes it. **The §4 rule 10 proof for this task is the vacuity proof** (D228(5)): a whole-schema delta over all 56 `public` tables across the whole lifecycle shows changes confined to **the container's own `leagues` row AND its single `league_members` row** (item 1's required write — an allowed-delta set that omits it makes the assertion RED on the task's own necessary work, and the cheap repair is to loosen the whole thing), the mock's `drafts`/`draft_picks`/`draft_bids`/`draft_queues`/`league_chat` rows, and the `cpu_seats` teams — **nothing else, in either direction**. Plus: no notification row, no `league_rosters` row, no `transactions` row, and no broadcast on any `league:<id>` topic but the container's own.
...
5. **The exits.** A standalone mock's room and report must not offer *"Back to league"* pointing at a container the user never joined (**five links, enumerated rather than sampled** — `grep -n "Back to league" src/components/draft/draft-recap.tsx src/components/draft/mock-draft-launcher.tsx` → `draft-recap.tsx:143`, `:235`, `:555` and `mock-draft-launcher.tsx:103`, `:119`). Standalone exits go to `/app/mocks`. `room-exits.test.ts` and `room-entry.test.ts` pin the current inventory by count — **re-point them, do not delete them.**
...
2. **A real table with column headers.** **Auction:** player · position · team · **price** · **nomination #**, plus a per-team spend summary from `recapTeamSpend` (`draft-recap-ops.ts:130–152`). **Snake/linear:** player · position · team · **round** · **pick #**. **Never one table with blank columns** (D230(2)).
...
6. **The delete affordance travels with the report** — `delete_mock_draft` is launcher-keyed at `071:500–504`; the UI must not offer it more widely (`recapVariant`'s `canDelete`, `draft-recap-ops.ts:39–54`, the flag itself at `:52`).
...
3. **Delete the dead disabled branch** (`:538–545`) — `MOCK_LAUNCHER_READY` has been `true` since L.B3.5 (`mock-launcher-entry.ts:10`), so that arm is unreachable. **This is the one in-file tidy this lane authorises**, because the task is already rewriting the component.
...
### MP.10 — The mock room leaves `/app/leagues` *(added by review — R470; a PREREQUISITE of MP.4, not a polish task)*

> Read this doc **§1.8**, **D231(2)–(3)**, **D228**; spec **v2.16 §8.8**'s independence bullet and **E79**. **Numbered last, sequenced early** — the MS.7/MS.8 precedent (a task added in an amendment round that gates an earlier-numbered one). **Depends on MP.3** for the flag and the `/app/mocks` family.

1. **The defect this closes.** `(room)/leagues/layout.tsx:18` hard-redirects **every** `/app/leagues` URL when `featureFlags.leagues` is off — the live draft room included, by design and by its own docblock. The mock room is that route (`mock-launcher-entry.ts:18`). **So the lane's headline property — practice works with leagues off — is refused at the route layer no matter what `featureFlags.mockDrafts` says**, and **E79 is LAW that nothing could satisfy** until this lands.
2. **Give the mock room its own route, keyed on the MOCK's id:** `/app/mocks/[mockId]` (the room) beside `/app/mocks` (the list, MP.3) and `/app/mocks/[mockId]/report` (MP.5). Gated on **`mockDrafts`**, and on nothing else.
3. **This also removes the container from the address bar.** Under D228 the standalone room's URL would otherwise be `/app/leagues/<container-uuid>/draft`, publishing the id of the row D228(1) calls hidden. **Keying on the mock's id is not a convenience — it is what makes "hidden" true.**
4. **Re-point, do not fork.** `mockLauncherHref` (`mock-launcher-entry.ts:13–19`), `room-entry.test.ts` and `room-exits.test.ts` (both pin the entry/exit inventory **by count**), and `route-groups.test.ts:223–232`, which gains the mock room's own gate assertion beside the two leagues ones. **A second room component is the LV.7 failure pattern**; this is a route move around the same component.
5. **The league-attached launcher may stay where it is.** Launching from League Home is a leagues-gated page by nature (D231(5)); what must not be gated is **the room**. If keeping the launcher at `?practice=1` while the room moves produces an awkward seam, say so in the PR and propose the whole mock surface moving — **but do not decide that silently**.
6. **§4 rule 15 is NOT engaged and this task says so out loud.** DR2 owns the room's **layout**; MP.10 moves the **route wrapper** (`(room)/leagues/[leagueId]/draft/page.tsx` is 20 lines and mounts `DraftRoom`). **Measured: PR #180 touches no code at all** — `git diff main...pr180 --stat` is five files, all under `docs/`. A Builder must not stop on rule 15 here; if the *component* needs restructuring to serve two routes, **that** is the DR2 question.
7. **The pins.** With `leagues` **off** and `mockDrafts` **on**: the mock room route renders (and the leagues room route still redirects — both, in the same test). **Show the new gate assertion RED** by pointing it at the un-gated route before adding the gate.
8. **Old URLs redirect, never 404** — the same obligation MP.8 carries for the recap, for the same reason: `?draft=<id>` room links exist in the wild (`mock-draft-launcher.tsx:333`).
...
**What becomes of AP.4.** Measured by the halted session and re-checked in §1.8: the league bid-clock default is already 20 (**9 occurrences, all 20**), the nomination clock is already 30 (**8, all 30**), the 10-second floor and the ranges already match §7.3.8. **Zero lines change for Q17's ruled half, and Q23's answer is that its unruled half is not built at all**, so **on this reading AP.4 closes with no code** and its remaining content moves to **MP.2 item 3** (the ten seconds, as a standardized-preset default) and **MS.3** (the in-room remedy). **B5 clears.**

**WHAT THIS PR ACTUALLY DOES TO AP.4's RECORD, stated precisely because an earlier draft of this section overclaimed it (R474).** It said the checklist row *"is marked closed-as-satisfied"*. **It is not: `git diff` shows zero edits to it, and the row on `main` (`PROGRESS:94`) still reads unchecked, still describes the mock override this section rules out, and still claims it discharges F85 — which §12 says it does not.** That edit is **deliberately held**: AP.4's disposition is with Chris (the reviewer's R472), and a lane document must not close another lane's task by assertion. **What this PR does is supply the ruling's basis and file the obligations**, so whoever applies the ruling has an actor and a list: the checklist row, ACTIVE-BUILD's loop step **1a**, and **F85's and F92's discharger cells, which both still name AP.4** — filed as **F106**. PR #186 is a documentation-only halt record and should be resolved as answered rather than left standing on a closed question.
...
MP.3 (the mockDrafts flag + /app/mocks + the user-scoped list)  ← independent of MP.1/MP.2*
  ├─→ MP.10 (the mock ROOM leaves /app/leagues)  ← R470; GATES MP.4
  │     └─→ MP.4 (Home entry point)  ←────────────┘ (also needs MP.2)
...
- **ONE HARD ORDERING CONSTRAINT, and like MS's it is a correctness one rather than a convenience: `MP.10` lands before `MP.4`.** Without it, `(room)/leagues/layout.tsx:18` redirects the mock room whenever the leagues flag is off (§1.8), so **MP.4's own `leagues`-off pin cannot pass and E79 is LAW nothing satisfies.** If the loop reaches MP.4 first, it must either take MP.10 in front of it or ship **without** the `leagues`-off pin and say so — and the second option is a strictly worse intermediate state than MS.5's, because the property being deferred is the lane's headline claim rather than one menu group.
...
- **Shared surfaces:** `PROGRESS-leagues.md` (append-collisions resolved at merge — the established pattern), the migration/pgTAP number band (§1.10 — confirm next-free at task time), `MockDraftLauncher` with MS.8, and `draft-recap*` with nothing currently in flight.
...
2. **Merge conflicts, per file and per resolution — and two of them are NOT "keep both" (R480/R481).**
   - **PR #180, spec `**Version:**` line** — `2.16` vs `2.14`. **"Keep both" is meaningless on a single-value header: TAKE THE HIGHER**, and let the changelog carry both entries.
   - **PR #180, changelog head and §19.2's tail** — genuinely append-shaped (DR2 appends E71–E74 where this PR appends E78–E80). **Keep both, in order.**
   - **PR #180, §8.8** — no conflict expected; DR2's nearest spec hunk was measured during the MS session at `@@ -570,14 @@`, which is §8.9. **Re-measure at merge rather than trusting that number.**
   - **PR #186, `PROGRESS` §4 and §7** — append-shaped (its D224 and its session-log row against this PR's D225–D232 and its own row). **Keep both.**
   - **PR #186, `PROGRESS` §5 — a CONTRADICTORY REPLACEMENT, not an append, and it must be resolved deliberately.** #186 asserts B5 as a standing blocker; this PR's §5 says B5 is cleared. **Both cannot stand.** If this PR merges second, its text wins and #186's blocker paragraph is dropped; if #186 merges second, whoever resolves must keep **this** PR's cleared text — **the question B5 stands on is answered, and a blocker outliving its question is exactly the stale-record shape F102 and F106 exist to prevent.**
   - **PR #186, `ACTIVE-BUILD.md` loop step 1a** — #186 rewrites it to *"blocked on Q23"*, which goes stale the moment Q23 is answered. **F102's fix shape covers it.**
...
| **F41** — §22.5 rate limits on draft actions | **Unchanged, and it now binds a second launch path.** MP.2's standalone launch counts against the **same** 3-active / 5-per-hour caps, counted the same way (`092:1246–1266`). **No new rate-limit surface**, and a Builder who adds one has changed §22.5 without a ruling. |
...
| **F98** — the MS lane clause in `ACTIVE-BUILD.md` | **Discharged 2026-08-21 by `1d4620d`, before this session — and this PR FLIPS ITS §6 STATUS CELL, which was left `⬜ Open` (R475).** The verification was already written into the row; the cell had simply not moved. §6 is the canonical ledger, so a discharged row reading Open is a live wrong answer to *"what still owes work?"*. Named here also because **F102 is the same obligation for MP** and a reader should see the precedent worked. |
...
| **F104** *(NEW — filed by this PR)* | **The two "More" lists are independently authored and nothing couples them.** `sidebar.tsx:57–74` (`MORE_ITEMS`, consumed only at `:141`/`:250` — measured, `grep -n "MORE_ITEMS" -r src` → 3 lines) and `more-sheet.tsx:35–64` (inline `<Row>`s). **The split itself is deliberate and form-factor driven and is NOT a defect** (§1.6) — the defect is that an edit to one **silently misses the other with no type error and no test to catch it**. **Fix shape:** one cheap source-text pin asserting the two agree about the entries common to both, in the `elevation-rule.test.ts` / `draft-command-bar.test.ts:151–153` idiom. **Discharged by MP.6**, which edits both and must show the pin RED by adding the row to one file only. |
...
| **F106** *(NEW — filed by this PR, R474)* | **F85's and F92's discharger cells both name `AP.4`, and §8 rules AP.4 has nothing left to build — so on that ruling both rows go OWNERLESS.** Measured: `PROGRESS:1056` col 5 reads *"**AP.4**, once Q17 is ruled"* (F85) and `PROGRESS:1058` col 5 reads *"**AP.4's re-measurement should quantify it again at the new clock**…"* (F92); both rows are `⬜ Open`. **This PR deliberately does NOT re-point them**, because AP.4's disposition is Chris's (the reviewer's R472) and re-pointing them here would settle by edit a question that is open by escalation. **Fix shape:** whoever applies the R472 ruling re-points F85's discharger to **MP.2** (the standardized preset supplies the necessary half) **plus F92** (the sufficient half), and F92's to its own follow-up rather than to a closed task — **or, if Chris rules AP.4 stays open, leaves both exactly as they are and closes this row as not-applicable.** **Discharged by a NAMED ACTOR: whoever applies Chris's AP.4 ruling, in the same commit as the checklist row (F102's actor, if it is the same person).** Filed rather than left implicit because R51 makes an unrecorded hand-off a finding in itself — and because a ledger row pointing at a closed task is the exact stale-record shape §6 exists to prevent. |
