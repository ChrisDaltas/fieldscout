...
| `draft_queues.team_id` | `UUID REFERENCES teams(id) ON DELETE CASCADE NOT NULL` — `065:194` | the queue is seat-keyed |
| `league_chat.league_id` | `UUID REFERENCES leagues(id) ON DELETE CASCADE NOT NULL` — `001:529` | the mock room's chat has no home |

- **RLS is league-keyed on every one of them.** `drafts` SELECT = `is_league_member(league_id)` (`065:139–140`); `draft_picks` the same (`065:175–176`); `draft_queues`, `draft_bids` and `league_chat` follow the same pattern. **Measured:** `CREATE POLICY … is_league_member` appears **13** times across the migration tree (`grep -rn "CREATE POLICY" -A 3 supabase/migrations/*.sql | grep -c "is_league_member"`).
...
2. **They must NOT be league rows.** `add_placeholder_seat` writes `teams` **and** `league_members` into the real league (`063:453–466`) — a §8.8 breach by construction, and it changes the D47/F28 capacity predicate every other seat RPC re-reads.
...
3. **The user-scoped read is new work.** `useMockDrafts` is keyed on a league (`use-mock-drafts.ts:41–48`) and `listMockDrafts` takes a `leagueId` (`draft-service.ts:1411`). Add the user-scoped sibling — `GET /api/mocks` — reusing the same column selection and the same launcher-scoped server-side filter. **Do not widen the league route to mean "all"**; two questions, two endpoints.
...
1. **The affordance sits in the existing Home structure** (§4 rule 15) — the natural place is `HomeQuickActions`' chip row (`home-quick-actions.tsx:46–91`), which already renders one un-gated chip (List) beside two `leagues`-gated ones. **A new hub card is a layout decision and belongs to DR2/the Figma, not here.**
...
3. **Delete the dead disabled branch** (`:539–545`) — `MOCK_LAUNCHER_READY` has been `true` since L.B3.5 (`mock-launcher-entry.ts:11`), so that arm is unreachable. **This is the one in-file tidy this lane authorises**, because the task is already rewriting the component.
...
**Why it is a question.** Chris's two rulings point at two different places and both are reasonable readings. Ruling 3.6: *"mocks should go to something on the **User's profile**, not at the league level."* Ruling 3.7: *"under the **More...** section there can be a Mock Drafts tab."* The first names the profile — and FieldScout profiles are **public, server-rendered** surfaces (`/u/[username]`, CLAUDE.md's SEO rule; the sidebar's own *My profile* row links there — `more-sheet.tsx:99–105`). The second names More…, which is **private app chrome**. **A mock report shows your strategy** — which players you paid up for, where you reached — and that is competitive information a league-mate could read the week before draft night.
