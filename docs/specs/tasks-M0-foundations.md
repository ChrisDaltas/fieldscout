...
- **Migrations:** `NNN_snake_case.sql`, 3-digit sequential (next = **040** — 037 = explicit grants, 038 = R5 notify_list_followers hardening, 039 = `nfl_weeks` landed 2026-07-19; this survey line originally said 037 and went stale through both renumberings, R20), banner comment citing spec §, RLS + indexes in the same file, `IF NOT EXISTS` guards, service-role-managed tables get SELECT-only policies + a comment. No `supabase/config.toml`, **no local stack, no pgTAP anywhere** — bootstrapped in L.A0.5a.
