...
> **Architect session output — 2026-07-18** (adversarially reviewed same session; findings applied). Read together with `spec-redraft-leagues.md` **v2.6.1** (the LAW) and `delivery-plan-redraft-leagues.md` v1.2. This doc sequences M0 into Builder-sized tasks; it never overrides the spec. Builder sessions take **one task each**, in dependency order (§5), and satisfy delivery plan §2.3 DoD per task.
>
> **Open Spec Questions gate some tasks** — see PROGRESS §3 (Q1, Q2) and the per-task "gated by" notes. Per CLAUDE.md, a Builder whose task touches an open question stops and waits.
...
- **`nfl_games` exists (001) but nothing writes to it.** Column is `kickoff_at` (spec erratum v2.6.1). Live sync approximates game windows from Sleeper's day-granularity `date` strings + a 36h constant. The free Sleeper schedule feed has **no kickoff timestamps** and there is **no official-inactives feed** → **PROGRESS Spec Question Q1**.
...
export interface StatKeyDef {
  key: string          // canonical key — ONE namespace shared by rules and stats (§7.3.3; Q2)
  label: string
  tier: StatTier
  storage: 'column' | 'advanced' | 'deferred'  // 'deferred' = no storage mapping until M1 (D5)
}
export const STAT_KEYS: readonly StatKeyDef[]  // seeded from Appendix B.1–B.5 + tracking/charted keys
```
...
Gating: **Q2 gates L.A0.2a's advanced-key seeding, L.A0.3's advanced payloads, and L.A0.4's fixture check-ins** (key names bake into artifacts). **L.A0.1 and L.A0.5a/5b are ungated** and can start immediately.
...
### L.A0.2a — StatsProvider contract + STAT_KEYS + Sleeper adapter *(gated by Q2 for advanced keys)*
> Build the provider contract as pure new code — **no existing file changes** beyond exports. Read spec §23.1, §23.5, Appendix B, delivery plan §2.1–2.3, this doc §3–4, PROGRESS Q1/Q2. Depends on L.A0.1.
>
> 1. `src/lib/leagues/stats/stat-keys.ts` — registry per D5: canonical keys from Appendix B.1–B.5 (+ the Q2-resolved tracking/charted keys), each with `label`, `tier`, `storage` (`'deferred'` where no `player_stats` column exists).
...
> 4. Tests: adapter mapping from trimmed-real Sleeper rows (inline, `sleeper.test.ts` style) → canonical payloads; registry invariants (unique keys, every Appendix B.1–B.5 key present, tier/storage consistency).
...
### L.A0.3 — SyntheticStatsProvider + scenario library (task lineage "L.A0b") *(gated by Q2; depends on L.A0.2a)*
> Build the synthetic tier. Read spec §23.6, §23.2 (outage/`stats_degraded`), §23.5 (tiers/pending), §19.2 rows E42–E45 + E55–E58, this doc §3–4.
>
> 1. `src/lib/leagues/stats/synthetic/`: seeded deterministic PRNG (no `Math.random` — same lint spirit as D3), versioned scenario schema, `SyntheticStatsProvider` pure in (scenario, seed, `time.now()`); fabricates core_box + tracking + charted lines and inactives ~90 virtual minutes pre-kickoff (§23.1).
...
### L.A0.4 — Fixture recorder + replay provider + capture CLI *(gated by Q2 for check-ins; depends on L.A0.2a — parallel-safe with L.A0.3)*
> Build the record/replay pair. Read spec §23.6 ("the recorder is the 'capture real'"), §23.2, delivery plan §3 (recording season) + §4.3, this doc D6–D7 + PROGRESS Q1.
>
...
| Recorder captures 2026 provider responses + injury/inactive feeds (recording season, plan §3) | L.A0.4 CLI live from September — **with the Q1 caveat: no official inactives / kickoff timestamps from sleeper_free until Q1 is decided** |
...
- **Q1 (PROGRESS §3):** `sleeper_free` has no kickoff timestamps (day-granularity schedule), no official-inactives feed; §23.1 makes both a hard requirement of whichever **live** provider runs, §23.3 locks derive from `nfl_games.kickoff_at`, and the M0 recorder inherits the gap (empty inactives in recordings). Wants an answer **before the September recording season**; hard-required before `sleeper_free` hosts real leagues (M4 gates on recorded-kickoff locks).
- **Q2 (PROGRESS §3):** canonical advanced stat-key names — §23.5 vs Appendix B.2 conflict. Gates L.A0.2a/3/4 as noted.
...
- **`nfl_games` is unwritten** — populating it (kickoffs, statuses) belongs to the milestone that first needs it at runtime, fed by whatever Q1 decides.
- **`player_stats.advanced` JSONB** (§23.5) — added by the first milestone that persists advanced stats (M1 Architect decides whether the Alpha/Ultra backtest persists or reads fixtures directly).
