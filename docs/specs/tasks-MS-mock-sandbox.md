...
- **`draft_set_order` is the one exception, and it is the lane's FOURTH structural breach — it is MIS-TARGETED, not blocked.** The order edit is not in `ControlRpcName`; it rides the draft **PATCH** (`patchDraft`, spec erratum v2.8.17/D114). **Its request carries no `draft_id`** — `orderRequest(leagueId, order, reason)` builds `{ order, reason }` and nothing else (`use-draft-controls-ops.ts:247–256`), and it is **the only draft control that omits one: measured 11/11** for the `dispatchControl` verbs (`awk` over every `…Request(` builder in that file; `memberAutodraftRequest` also omits one, and it is §2.2's other breach). So `patchDraft` picks the target itself — `.eq('league_id', leagueId).eq('is_mock', false).in('status', ACTIVE_DRAFT_STATUSES)` at **`draft-service.ts:198–204`** — and hands *that* id to the RPC at **`draft-service.ts:260–264`** (`p_draft_id: draft.id`).
- **The consequence, and it is the opposite of harmless.** `draft_set_order`'s own mock refusal exists in `prosrc` and is at `087:2341` — **and can never fire, because the RPC is never handed the mock.** A commissioner practising in their own league who edits the order from the mock room rewrites **the REAL draft's order** and posts *"Draft order updated by …"* into **the real draft's room**. **Probed here** (rolled-back txn, minimal fixture — one league, a real `live` draft `…d1`, a mock `live` draft `…d2`, then `patchDraft`'s resolver verbatim): the resolver returns **`…d1`, the REAL draft**, never the mock. An RPC-level fix therefore cannot work — **the exclusion has to be a service-layer block** (§5 MS.2 item 5).
...
| 9 | **Edit draft / nomination order** `draft_set_order` | 087 | `drafts`, `league_chat` | `draft_team_for_pick` | ❌ **BREACH — MIS-TARGETED (§1.3).** The RPC body is mock-local and its mock refusal exists, but `orderRequest` sends **no `draft_id`** (`use-draft-controls-ops.ts:247–256`) so `patchDraft` resolves the **real** draft (`draft-service.ts:198–204`) and passes that id in (`:260–264`). The guard never fires; the write lands on the real draft. Probed. |
...
**FOUR of the fifteen are structural §8.8 breaches — `draft_reset`, the autopick toggle, seat reassignment, and the mis-targeted `draft_set_order` — not one.** The working draft of this document assumed `draft_reset` might be the only one; it was not, and the fourth is the worst of them **because it is not blocked at all**: its guard exists, passes a `prosrc` inspection, and never runs (§1.3). Everything else in the table is *provisionally* mock-local **by code read**, which is not the standard: **MS.1 converts every "mock-local" cell above into a measurement or removes it from the cleared list — and it must measure through the ROUTE, not only through the RPC, or it cannot see this class at all (§5 MS.1 item 3).**
...
> Read spec **v2.15 §8.7** (the table + the v2.15 mock note) and **§8.8**, E75; this doc **§1.1, §1.2, §1.3, §2, D217**; `069:396–415` and `069:457–468` (the idiom), the eleven HEAD sites of §1.1, `draft-service.ts:196–215` (`patchDraft`'s mock filter) and `:1074–1094` (`dispatchControl`); **PROGRESS D103 / D110(1) / D138.** **Depends MS.1** — it enables exactly MS.1's cleared list and nothing else.
>
...
> 5. **`draft_set_order` is EXCLUDED, and the exclusion must be a SERVICE-LAYER block — an RPC message change cannot work here.** §1.3: `orderRequest` sends no `draft_id`, so `patchDraft` resolves the target itself and hands the **real** draft's id to the RPC. The RPC's mock refusal is therefore **never reached**, and re-wording it (item 4) would change nothing while looking like a fix. **`patchDraft` must refuse when the room is a mock** — the order PATCH takes the mock's `draft_id` from the caller and refuses it with a service-layer message, or the mock room does not offer the control and the service refuses the call anyway. **Both halves are required:** MS.5 must not render it (item 6 there), *and* the server must refuse it, because §12's server-authoritative rule means a UI-only exclusion is not an exclusion. Pin it end-to-end through the route, with **a real draft alive in the same league**, asserting the real draft's `draft_order` and its chat context are byte-identical afterwards — the exact probe §1.3 ran. *(If a later task wants a mock's order to be editable, the honest shape is to give `orderRequest` a `draft_id` and route it through `dispatchControl` like its eleven siblings. That is **not** this lane's work.)*
