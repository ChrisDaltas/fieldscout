/**
 * POST /api/leagues/[id]/commish/bracket — the thin handler's own contract (M6A
 * L.E1.16, D351): a malformed league id is a 404 and an unauthenticated
 * caller a 401 BEFORE the service is reached; an authenticated caller's body
 * is handed to `commishEditBracket` whole with the injected client, and the
 ...
 * `commish-bracket-service.test.ts`.
 */
...
const commishEditBracket = vi.fn()
vi.mock('@/lib/leagues/api/commish-bracket-service', () => ({ commishEditBracket: (...args: unknown[]) => commishEditBracket(...args) }))
...
const post = (body: unknown) =>
  new Request('http://x/api/leagues/' + LEAGUE + '/commish/bracket', { method: 'POST', body: JSON.stringify(body) })

describe('POST /api/leagues/[id]/commish/bracket', () => {
  it('refuses an unauthenticated caller with 401 before the service is built or called', async () => {
    ...
    expect(commishEditBracket).not.toHaveBeenCalled()
  })
  ...
  it('an authenticated caller’s body reaches the service WHOLE with the injected client, and the service’s answer is the response', async () => {
    ...
    commishEditBracket.mockResolvedValueOnce({ status: 409, body: { error: 'the verb’s own refusal, verbatim' } })
    ...
    expect(commishEditBracket).toHaveBeenCalledTimes(1)
    const [client, leagueId, passed] = commishEditBracket.mock.calls[0]
  ...
  it('an unparseable body reaches the service as null (a 400 there), never a 500 here', async () => {
    getUser.mockResolvedValueOnce({ data: { user: { id: 'u1' } } })
    commishEditBracket.mockResolvedValueOnce({ status: 400, body: { error: { fieldErrors: {} } } })
    const res = await POST(new Request('http://x', { method: 'POST', body: '{nope' }), params(LEAGUE))
    expect(res.status).toBe(400)
    expect(commishEditBracket.mock.calls.at(-1)?.[2]).toBeNull()
  })
})
