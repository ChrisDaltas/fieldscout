/**
 * POST /api/leagues/[id]/commish/schedule — the thin handler's own contract (M6A
 ...
 * is handed to `commishEditSchedule` whole with the injected client, and the
 ...
 * `commish-schedule-service.test.ts`.
 */
...
const commishEditSchedule = vi.fn()
vi.mock('@/lib/leagues/api/commish-schedule-service', () => ({ commishEditSchedule: (...args: unknown[]) => commishEditSchedule(...args) }))
...
const post = (body: unknown) =>
  new Request('http://x/api/leagues/' + LEAGUE + '/commish/schedule', { method: 'POST', body: JSON.stringify(body) })

describe('POST /api/leagues/[id]/commish/schedule', () => {
  it('refuses an unauthenticated caller with 401 before the service is built or called', async () => {
    ...
    expect(commishEditSchedule).not.toHaveBeenCalled()
  })
  ...
  it('an authenticated caller’s body reaches the service WHOLE with the injected client, and the service’s answer is the response', async () => {
    ...
    commishEditSchedule.mockResolvedValueOnce({ status: 409, body: { error: 'the verb’s own refusal, verbatim' } })
    ...
    expect(commishEditSchedule).toHaveBeenCalledTimes(1)
    const [client, leagueId, passed] = commishEditSchedule.mock.calls[0]
  ...
  it('an unparseable body reaches the service as null (a 400 there), never a 500 here', async () => {
    getUser.mockResolvedValueOnce({ data: { user: { id: 'u1' } } })
    commishEditSchedule.mockResolvedValueOnce({ status: 400, body: { error: { fieldErrors: {} } } })
    const res = await POST(new Request('http://x', { method: 'POST', body: '{nope' }), params(LEAGUE))
    expect(res.status).toBe(400)
    expect(commishEditSchedule.mock.calls.at(-1)?.[2]).toBeNull()
  })
})
