/**
 * POST /api/leagues/[id]/commish/roster — the thin handler's own contract (M6A
 * L.E1.10, D351): a malformed league id is a 404 and an unauthenticated
 * caller a 401 BEFORE the service is reached; an authenticated caller's body
 * is handed to `commishForceAddDrop` whole with the injected client, and the
 ...
 * `commish-roster-service.test.ts`.
 */
...
const commishForceAddDrop = vi.fn()
vi.mock('@/lib/leagues/api/commish-roster-service', () => ({ commishForceAddDrop: (...args: unknown[]) => commishForceAddDrop(...args) }))
...
const post = (body: unknown) =>
  new Request('http://x/api/leagues/' + LEAGUE + '/commish/roster', { method: 'POST', body: JSON.stringify(body) })

describe('POST /api/leagues/[id]/commish/roster — the force add / drop', () => {
  it('refuses an unauthenticated caller with 401 before the service is built or called', async () => {
    ...
    expect(commishForceAddDrop).not.toHaveBeenCalled()
  })
  ...
  it('an authenticated caller’s body reaches the service WHOLE with the injected client, and the service’s answer is the response', async () => {
    ...
    commishForceAddDrop.mockResolvedValueOnce({ status: 409, body: { error: 'the verb’s own refusal, verbatim' } })
    ...
    expect(commishForceAddDrop).toHaveBeenCalledTimes(1)
    const [client, leagueId, passed] = commishForceAddDrop.mock.calls[0]
  ...
  it('an unparseable body reaches the service as null (a 400 there), never a 500 here', async () => {
    getUser.mockResolvedValueOnce({ data: { user: { id: 'u1' } } })
    commishForceAddDrop.mockResolvedValueOnce({ status: 400, body: { error: { fieldErrors: {} } } })
    const res = await POST(new Request('http://x', { method: 'POST', body: '{nope' }), params(LEAGUE))
    expect(res.status).toBe(400)
    expect(commishForceAddDrop.mock.calls.at(-1)?.[2]).toBeNull()
  })
})
