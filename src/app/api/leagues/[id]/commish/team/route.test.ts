/**
 * POST /api/leagues/[id]/commish/team — the thin handler's own contract (M6A
 ...
 * is handed to `commishRenameTeam` whole with the injected client, and the
...
const commishRenameTeam = vi.fn()
vi.mock('@/lib/leagues/api/commish-team-service', () => ({ commishRenameTeam: (...args: unknown[]) => commishRenameTeam(...args) }))
...
const post = (body: unknown) =>
  new Request('http://x/api/leagues/' + LEAGUE + '/commish/team', { method: 'POST', body: JSON.stringify(body) })

describe('POST /api/leagues/[id]/commish/team', () => {
  it('refuses an unauthenticated caller with 401 before the service is built or called', async () => {
    ...
    expect(commishRenameTeam).not.toHaveBeenCalled()
  })
  ...
  it('an authenticated caller’s body reaches the service WHOLE with the injected client, and the service’s answer is the response', async () => {
    ...
    commishRenameTeam.mockResolvedValueOnce({ status: 409, body: { error: 'the verb’s own refusal, verbatim' } })
    ...
    expect(commishRenameTeam).toHaveBeenCalledTimes(1)
    const [client, leagueId, passed] = commishRenameTeam.mock.calls[0]
  ...
  it('an unparseable body reaches the service as null (a 400 there), never a 500 here', async () => {
    getUser.mockResolvedValueOnce({ data: { user: { id: 'u1' } } })
    commishRenameTeam.mockResolvedValueOnce({ status: 400, body: { error: { fieldErrors: {} } } })
    const res = await POST(new Request('http://x', { method: 'POST', body: '{nope' }), params(LEAGUE))
    expect(res.status).toBe(400)
    expect(commishRenameTeam.mock.calls.at(-1)?.[2]).toBeNull()
  })
})
