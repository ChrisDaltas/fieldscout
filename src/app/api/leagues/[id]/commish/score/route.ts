...
import { commishEditScore } from '@/lib/leagues/api/commish-matchup-service'
 ...
 *  (spec §15.4:1692 → `commish_edit_score`; §10.3; M6A L.E1.10, PROGRESS D351).
 *
 ...
 *
 *  Body: { matchup_id, home_score, away_score, action_id, reason? } — one UUID per submit (re-sending the same body replays it).
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const result = await commishEditScore(supabase, id, body)
