/**
 * POST /api/leagues/[id]/commish/score — the thin handler's own contract (M6A
 ...
 * is handed to `commishEditScore` whole with the injected client, and the
...
const commishEditScore = vi.fn()
vi.mock('@/lib/leagues/api/commish-matchup-service', () => ({ commishEditScore: (...args: unknown[]) => commishEditScore(...args) }))
...
const post = (body: unknown) =>
  new Request('http://x/api/leagues/' + LEAGUE + '/commish/score', { method: 'POST', body: JSON.stringify(body) })

describe('POST /api/leagues/[id]/commish/score — the score correction', () => {
  it('refuses an unauthenticated caller with 401 before the service is built or called', async () => {
    ...
    expect(commishEditScore).not.toHaveBeenCalled()
  })
  ...
  it('an authenticated caller’s body reaches the service WHOLE with the injected client, and the service’s answer is the response', async () => {
    ...
    commishEditScore.mockResolvedValueOnce({ status: 409, body: { error: 'the verb’s own refusal, verbatim' } })
    ...
    expect(commishEditScore).toHaveBeenCalledTimes(1)
    const [client, leagueId, passed] = commishEditScore.mock.calls[0]
  ...
  it('an unparseable body reaches the service as null (a 400 there), never a 500 here', async () => {
    getUser.mockResolvedValueOnce({ data: { user: { id: 'u1' } } })
    commishEditScore.mockResolvedValueOnce({ status: 400, body: { error: { fieldErrors: {} } } })
    const res = await POST(new Request('http://x', { method: 'POST', body: '{nope' }), params(LEAGUE))
    expect(res.status).toBe(400)
    expect(commishEditScore.mock.calls.at(-1)?.[2]).toBeNull()
  })
})
