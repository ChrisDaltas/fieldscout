/**
 * POST /api/leagues/[id]/commish/move-player — the thin handler's own contract (M6A
 ...
 * is handed to `commishMovePlayer` whole with the injected client, and the
...
const commishMovePlayer = vi.fn()
vi.mock('@/lib/leagues/api/commish-roster-service', () => ({ commishMovePlayer: (...args: unknown[]) => commishMovePlayer(...args) }))
...
const post = (body: unknown) =>
  new Request('http://x/api/leagues/' + LEAGUE + '/commish/move-player', { method: 'POST', body: JSON.stringify(body) })

describe('POST /api/leagues/[id]/commish/move-player — the player move', () => {
  it('refuses an unauthenticated caller with 401 before the service is built or called', async () => {
    ...
    expect(commishMovePlayer).not.toHaveBeenCalled()
  })
  ...
  it('an authenticated caller’s body reaches the service WHOLE with the injected client, and the service’s answer is the response', async () => {
    ...
    commishMovePlayer.mockResolvedValueOnce({ status: 409, body: { error: 'the verb’s own refusal, verbatim' } })
    ...
    expect(commishMovePlayer).toHaveBeenCalledTimes(1)
    const [client, leagueId, passed] = commishMovePlayer.mock.calls[0]
  ...
  it('an unparseable body reaches the service as null (a 400 there), never a 500 here', async () => {
    getUser.mockResolvedValueOnce({ data: { user: { id: 'u1' } } })
    commishMovePlayer.mockResolvedValueOnce({ status: 400, body: { error: { fieldErrors: {} } } })
    const res = await POST(new Request('http://x', { method: 'POST', body: '{nope' }), params(LEAGUE))
    expect(res.status).toBe(400)
    expect(commishMovePlayer.mock.calls.at(-1)?.[2]).toBeNull()
  })
})
