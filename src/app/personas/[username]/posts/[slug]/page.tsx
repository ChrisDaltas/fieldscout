...
async function loadPost(username: string, slug: string) {
  ...
  const { data: persona } = await supabase
    .from('ai_personas')
    .select('id, username, persona_name, avatar_url')
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${post.title} · ${persona.persona_name} · FieldScout`,
...
export default async function PersonaPostPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    author: {
      ...
      description: personaDisclaimer(persona.persona_name),
    },
  }
    ...
    <GuestShell>
      <article className="mx-auto max-w-2xl">
        ...
        <p className="mt-6 border-t border-n-4 pt-4 text-[11px] font-medium text-n-3">
          {personaDisclaimer(persona.persona_name)} Generated by FieldScout AI.
        </p>
      </article>
    </GuestShell>
  )
