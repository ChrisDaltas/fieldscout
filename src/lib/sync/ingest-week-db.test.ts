 ...
 *
 * Fixture hygiene (F199): the 18 synthetic players are inserted here and
 * removed after; every touched row (score_fanout, player_stats, nfl_games,
 * the (2026, 2) nfl_weeks bounds) is restored — pgTAP 003 pins those bounds
 * NULL, so `afterAll` MUST put them back. If the stack is down this suite
 * FAILS (never skips — D59).
 */
...
async function cleanup(): Promise<void> {
  ...
  await must(service.from('nfl_games').delete().like('id', '2026-wk02-%'), 'cleanup nfl_games')
  await must(
    service
      .from('nfl_weeks')
...
async function storedGames(): Promise<Map<string, { kickoff_at: string; status: string; updated_at: string }>> {
  const rows = await must(
    service.from('nfl_games').select('id, kickoff_at, status, updated_at').like('id', '2026-wk02-%').order('id'),
    'read nfl_games',
  )
  return new Map(
...
async function queued(): Promise<string[]> {
  const rows = await must(
    service.from('score_fanout').select('player_id, enqueued_at').eq('season', SEASON).eq('week', WEEK).order('player_id'),
    'read score_fanout',
  )
  return (rows as Array<{ player_id: string }>).map((r) => r.player_id)
}
...
describe('ingestWeek — the golden first poll and diff-awareness (§22.2/§23.2)', () => {
  it('writes nfl_games, nfl_weeks.first_kickoff_at, player_stats (+advanced, source, D12 stamp) and enqueues exactly the new rows', async () => {
    ...
    expect(report.weeks).toEqual({ touched: 1, updated: 1, unchanged: 0 })
  ...
  it('an IDENTICAL re-poll writes nothing and enqueues nothing — counted (the zero-delta pin; the DoD probe reds here)', async () => {
    ...
    expect(again.weeks).toEqual({ touched: 1, updated: 0, unchanged: 1 })
  ...
  it('a moved poll updates exactly the moved rows; an undrained queue dedupes (D292); a drained one re-enqueues', async () => {
    ...
    await must(service.from('score_fanout').delete().eq('season', SEASON).eq('week', WEEK), 'drain')
...
describe('E42 — flex_move moves nfl_games.kickoff_at at the announcement instant', () => {
  it('announceAt−1s: unchanged; announceAt: G3 moves Sun-night → Sun-late, G1/G2 untouched, first_kickoff_at stays G1', async () => {
    ...
    expect(await storedWeek()).toEqual({ first_kickoff_at: '2026-09-20T17:00:00.000Z', last_game_ends_at: null })
    expect(at.weeks).toEqual({ touched: 1, updated: 0, unchanged: 1 })
  })
})
...
describe('E43 / §12.20 — postponement and the last_game_ends_at rule', () => {
  ...
  it('last_game_ends_at is NULL one second before the last game ends and stamps at the ending instant (D146)', async () => {
    ...
    expect(at.weeks).toEqual({ touched: 1, updated: 1, unchanged: 0 })
    ...
    expect(later.weeks).toEqual({ touched: 1, updated: 0, unchanged: 1 })
...
describe('loud emptiness (CLAUDE.md) — every skip is counted and named', () => {
  ...
  it('a tier without kickoff timestamps (sleeper-shaped) writes no games and says why', async () => {
    ...
    expect(report.weeks).toEqual({ touched: 0, updated: 0, unchanged: 0 })
