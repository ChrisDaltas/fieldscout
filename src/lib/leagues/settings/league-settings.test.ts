...
describe('LEAGUE_SETTINGS_DEFAULTS (§7.3 "D" columns)', () => {
  ...
  // v2.16.21 (Q35 (a), Chris 2026-09-05; migration 115): the retired key is refused at parse — the catalog
  // is a strict object, so a stale client sending `player_game_lock` gets a 400 before the table's CHECK
  // (23514) ever sees it. Pinned so the key cannot quietly re-enter the schema.
  it('the retired player_game_lock key is refused at parse (strict object — migration 115 retires it)', () => {
    expect(parses(withPatch('player_game_lock', true))).toBe(false)
    expect(parses(withPatch('player_game_lock', false))).toBe(false)
