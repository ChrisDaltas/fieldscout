...
// are the key names the eight shipped templates and migrations 058 + 106 +
// 108's seeded rows actually carry (D62: golden pins are stored literals).
...
> = {
  // The Scout pair rides the ESPN split families — B.5's markup ruling
  // (SC.1); one shared body one value apart (B.5.1, SC.4).
  'Scout Standard': { pa: ESPN_PA_CUTS, ya: YA_CUTS },
  'Scout PPR': { pa: ESPN_PA_CUTS, ya: YA_CUTS },
...
describe('tierKeysFromCuts — the §7.3.3.1(a) generation rule', () => {
  ...
  it('generates exactly the def_pa_*/def_ya_* keys the EIGHT SHIPPED TEMPLATES actually reference', () => {
