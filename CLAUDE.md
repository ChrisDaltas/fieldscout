...
- **Single theme.** One mode blending dark and light. There is no dark/light toggle. Remove/ignore `next-themes` and `dark:` variants as screens are reskinned; never generate dual-theme tokens.
  - **One named carve-out: the live draft room is dark** *(Chris, 2026-08-20 — "yeah just this page definitely not the whole app. don't ever assume things like that"; spec-redraft-leagues.md §16.1 v2.14).* The `(room)` route group — the draft lobby and the live room, nothing else — renders on a dark neutral palette. **This is not a violation of the rule above, because the rule forbids a user-facing toggle and the doubled token set a toggle requires, and the room has neither:** it is dark for everyone, always, with no toggle, no stored preference, and **no `dark:` variant** (the project sets no `darkMode` key, so `dark:` would follow the viewer's OS — exactly what this is not). Only the **neutral** layer re-points; `accent`, `brand`, `positive`/`negative`/`caution`, `pos-*` and `tier-*` keep their app-wide values, because a position badge cannot mean one colour on Lists and another in the draft room. **Do not generalise this to any other screen** — that is the sentence Chris's ruling was about. Any further exception is his call, not an inference from this one.
